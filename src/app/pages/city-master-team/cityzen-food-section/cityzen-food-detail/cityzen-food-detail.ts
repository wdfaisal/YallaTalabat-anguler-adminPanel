...
import { CityzenFoodDetailReviewInterface, FoodHashtag } from 'src/app/interfaces/cityzen.food.detail.review.interface';
import { CityzenFoodDetailTaxationInterface } from 'src/app/interfaces/cityzen.food.detail.taxation.interface';
import { CityzenFoodDetailVariationInterface } from 'src/app/interfaces/cityzen.food.detail.varation.interface';
...
@Component({
  selector: 'app-cityzen-food-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-food-detail.html',
})
export class CityzenFoodDetail {

  @ViewChild('foodReviewPaginator', { read: MatPaginator, static: false }) foodReviewPaginator: MatPaginator;
  foodReview: CityzenFoodDetailReviewInterface[] = [];
  ...
  addons: string = '';
  variation = new MatTableDataSource<CityzenFoodDetailVariationInterface>([]);
  displayedVariationColumns: string[] = ['title', 'options'];
  taxation: CityzenFoodDetailTaxationInterface[] = [];
  ...
  getDetail() {
    ...
    this.isLoaded = false;
    this.api.get_private('v1/cityzen/food_detail/' + this.id + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.success == true) {
          ...
          if (detail && detail.foodtaxations && detail.foodtaxations.length) {
            const mappedList = detail.foodtaxations.map(
              (item: CityzenFoodDetailTaxationInterface) => {
            ...
            this.taxation = detail.foodtaxations;
          } else {
            this.taxation = [];
          }
