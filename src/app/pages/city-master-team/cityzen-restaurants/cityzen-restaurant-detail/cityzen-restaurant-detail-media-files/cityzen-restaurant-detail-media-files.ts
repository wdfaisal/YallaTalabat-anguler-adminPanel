...
@Component({
  selector: 'app-cityzen-restaurant-detail-media-files',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgScrollbarModule],
  templateUrl: './cityzen-restaurant-detail-media-files.html',
})
export class CityzenRestaurantDetailMediaFiles implements AfterViewInit {
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_media_files?' + httpParams.toString()).subscribe({
      next: (response: any) => {
