...
import { CityzenVendorFoodReviewListInterface, FoodHashtag } from 'src/app/interfaces/cityzen.vendor.food.review.list.interface';
import { CityzenVendorOrderReviewListInterface, RestaurantHashtag } from 'src/app/interfaces/cityzen.vendor.order.review.list.interface';
...
@Component({
  selector: 'app-cityzen-restaurant-detail-reviews',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-restaurant-detail-reviews.html',
})
export class CityzenRestaurantDetailReviews implements AfterViewInit {
  ...
  restaurantReview: CityzenVendorOrderReviewListInterface[] = [];
  foodReview: CityzenVendorFoodReviewListInterface[] = [];
  ...
  getInitial() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_all_reviews?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.food && response.food.foodReviews) {
          const mappedList = response.food.foodReviews.map(
            (item: CityzenVendorFoodReviewListInterface) => {
  ...
  getFoodReview() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_vendor_food_reviews?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedFood = true;
  ...
  getRestaurantReview() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_own_reviews?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedRestaurant = true;
        if (response && response.results) {
