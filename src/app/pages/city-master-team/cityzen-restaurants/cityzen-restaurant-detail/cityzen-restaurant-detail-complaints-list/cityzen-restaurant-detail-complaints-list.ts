...
import { CityzenVendorOrderComplaintInterface } from 'src/app/interfaces/cityzen.vendor.order.complaint.interface';
import { CityzenVendorRestaurantComplaintInterface } from 'src/app/interfaces/cityzen.vendor.restaurant.complaint.interface';
...
@Component({
  selector: 'app-cityzen-restaurant-detail-complaints-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-restaurant-detail-complaints-list.html',
})
export class CityzenRestaurantDetailComplaintsList implements AfterViewInit {
  ...
  userComplaintList = new MatTableDataSource<CityzenVendorOrderComplaintInterface>([]);
  restaurantComplaintList = new MatTableDataSource<CityzenVendorRestaurantComplaintInterface>([]);
  ...
  getInitial() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_complaint_list?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.order && response.order.orderComplaint) {
          const mappedList = response.order.orderComplaint.map(
            (item: CityzenVendorOrderComplaintInterface) => {
          ...
          this.userComplaintList = new MatTableDataSource<CityzenVendorOrderComplaintInterface>(mappedList);
        ...
        if (response && response.restaurant && response.restaurant.restaurantComplaint) {
          const mappedList = response.restaurant.restaurantComplaint.map(
            (item: CityzenVendorRestaurantComplaintInterface) => {
              if (item && item.reasons && item.reasons?.id) {
                if (item.reasons?.translations) {
                  const translation = item.reasons.translations.find((t) => t.code == this.util.appLocaleName());
                  item.reasons.displayName = translation?.value || item.reasons.name;
          ...
          this.restaurantComplaintList = new MatTableDataSource<CityzenVendorRestaurantComplaintInterface>(mappedList);
  ...
  getUserComplaint() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_order_complaint_list?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedUser = true;
  ...
  getRestaurantComplaint() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_own_complaint_list?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedRestaurant = true;
        if (response && response.results) {
  ...
  onUserComplaintInfo(item: CityzenVendorOrderComplaintInterface) {
    console.log(item);
  ...
  onUserOrderInfo(item: CityzenVendorOrderComplaintInterface) {
    console.log(item);
  ...
  onRestaurantOrderInfo(item: CityzenVendorRestaurantComplaintInterface) {
    console.log(item);
    if (item && item.orderInfo && item.orderInfo.id && item.orderInfo.id != '') {
      this.router.navigate(['cityzen-team/u/order-details', item.orderInfo.id]);
    }
  ...
  onUserOrderDeliverymanInfo(item: CityzenVendorOrderComplaintInterface) {
    console.log(item);
  ...
  onRestaurantDeliverymanInfo(item: CityzenVendorRestaurantComplaintInterface) {
    console.log(item);
    if (item && item.driverInfo && item.driverInfo.id && item.driverInfo.id != '') {
      this.router.navigate(['cityzen-team/u/deliveryman-details', item.driverInfo.id]);
    }
  ...
  onRestaurantUserInfo(item: CityzenVendorRestaurantComplaintInterface) {
    console.log(item);
    if (item && item.userInfo && item.userInfo.id && item.userInfo.id != '') {
      this.router.navigate(['cityzen-team/u/customer-detail', item.userInfo.id]);
    }
