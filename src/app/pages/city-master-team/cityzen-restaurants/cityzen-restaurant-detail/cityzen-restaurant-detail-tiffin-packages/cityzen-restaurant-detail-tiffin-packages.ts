...
import { CityzenVendorTiffinSubscriptionPackageListInterface } from 'src/app/interfaces/cityzen.vendor.tiffin.subscription.package.list.interface';
...
@Component({
  selector: 'app-cityzen-restaurant-detail-tiffin-packages',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-restaurant-detail-tiffin-packages.html',
})
export class CityzenRestaurantDetailTiffinPackages implements AfterViewInit {

  @Input() restaurantId!: string;
  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  packages = new MatTableDataSource<CityzenVendorTiffinSubscriptionPackageListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_tiffin_packages_list?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: CityzenVendorTiffinSubscriptionPackageListInterface) => {
          ...
          this.packages = new MatTableDataSource<CityzenVendorTiffinSubscriptionPackageListInterface>(mappedList);
  ...
  onEdit(packageItem: CityzenVendorTiffinSubscriptionPackageListInterface) {
    console.log(packageItem);
    this.router.navigate(['cityzen-team/u/manage-subscription-tiffin-package/', packageItem.id, packageItem.restaurant.id]);
  }

  onPurchasedList(packageItem: CityzenVendorTiffinSubscriptionPackageListInterface) {
    console.log(packageItem);
    this.router.navigate(['cityzen-team/u/user-purchased-tiffin-subscription-list/', packageItem.id]);
  }

  onStatusChange(event: MatSelectChange, packageItem: CityzenVendorTiffinSubscriptionPackageListInterface) {
    console.log(event);
  ...
  onDelete(packageItem: CityzenVendorTiffinSubscriptionPackageListInterface) {
