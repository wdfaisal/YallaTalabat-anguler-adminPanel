...
import { CityzenVendorFoodListInterface } from 'src/app/interfaces/cityzen.vendor.food.list.interface';
...
@Component({
  selector: 'app-cityzen-restaurant-detail-food-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-restaurant-detail-food-list.html',
})
export class CityzenRestaurantDetailFoodList implements AfterViewInit {

  @Input() restaurantId!: string;
  @ViewChild('paginator', { read: MatPaginator }) paginator: MatPaginator;
  foods = new MatTableDataSource<CityzenVendorFoodListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/restaurant_detail_food_list?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: CityzenVendorFoodListInterface) => {
          ...
          this.foods = new MatTableDataSource<CityzenVendorFoodListInterface>(mappedList);
  ...
  onStockChange(event: MatSlideToggleChange, food: CityzenVendorFoodListInterface) {
    console.log(event);
    console.log(food);
  ...
  onEdit(food: CityzenVendorFoodListInterface) {
    this.router.navigate(['cityzen-team/u/edit-food', food.id]);
  }

  onDelete(food: CityzenVendorFoodListInterface) {
  ...
  onStatusChange(event: MatSelectChange, food: CityzenVendorFoodListInterface) {
    console.log(event);
  ...
  onFoodDetail(item: CityzenVendorFoodListInterface) {
    this.router.navigate(['cityzen-team/u/food-detail', item.id]);
  }
