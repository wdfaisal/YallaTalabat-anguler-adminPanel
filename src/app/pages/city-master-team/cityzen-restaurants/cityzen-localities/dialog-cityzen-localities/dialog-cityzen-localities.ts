...
@Component({
  selector: 'app-dialog-cityzen-localities',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-cityzen-localities.html',
})
export class DialogCityzenLocalities {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogCityzenLocalities>,
  ...
  onSubmit() {
    ...
    if (this.localityForm.valid) {
      if (this.action == 'create') {
        this.isSubmit = true;
        this.api.post_private('v1/cityzen/localities/save/' + this.util.getItem('_uid'), this.localityForm.getRawValue()).subscribe({
          next: (response: any) => {
        ...
        this.isSubmit = true;
        this.api.patch_private('v1/cityzen/localities/update/' + this.id, this.localityForm.getRawValue()).subscribe({
          next: (response: any) => {
            console.log(response);
            this.isSubmit = false;
