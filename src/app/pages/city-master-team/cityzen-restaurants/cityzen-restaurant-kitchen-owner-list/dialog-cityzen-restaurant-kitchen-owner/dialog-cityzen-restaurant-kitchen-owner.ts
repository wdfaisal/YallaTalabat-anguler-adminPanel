...
@Component({
  selector: 'app-dialog-cityzen-restaurant-kitchen-owner',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-cityzen-restaurant-kitchen-owner.html',
})
export class DialogCityzenRestaurantKitchenOwner {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/cityzen/kitchen_owner/info/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onSubmit() {
    ...
    this.api.patch_private('v1/cityzen/kitchen_owner/update_detail/' + this.userId, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
