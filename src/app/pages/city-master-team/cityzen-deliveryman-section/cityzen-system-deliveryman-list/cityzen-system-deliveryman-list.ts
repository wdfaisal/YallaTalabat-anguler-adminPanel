...
import { CityzenDriverListInterface } from 'src/app/interfaces/cityzen.driver.list.interface';
...
@Component({
  selector: 'app-cityzen-system-deliveryman-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-system-deliveryman-list.html',
})
export class CityzenSystemDeliverymanList {

  @ViewChild('paginator', { read: MatPaginator }) paginator: MatPaginator;
  drivers = new MatTableDataSource<CityzenDriverListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/system_deliveryman_list/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: CityzenDriverListInterface) => {
          ...
          this.drivers = new MatTableDataSource<CityzenDriverListInterface>(mappedList);
  ...
  onStatusChange(event: MatSlideToggleChange, driver: CityzenDriverListInterface) {
    console.log(event);
    console.log(driver);
  ...
  onBlockedChange(event: MatSlideToggleChange, driver: CityzenDriverListInterface) {
    console.log(event);
    console.log(driver);
  ...
  onEdit(driver: CityzenDriverListInterface) {
    console.log(driver);
  ...
  onAddNewDriver() {
    this.router.navigate(['cityzen-team/u/add-driver']);
  }

  onDeliverymanInfo(item: CityzenDriverListInterface) {
    console.log(item);
