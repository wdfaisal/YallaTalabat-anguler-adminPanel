...
@Component({
  selector: 'app-cityzen-deliveryman-detail-medias',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgScrollbarModule],
  templateUrl: './cityzen-deliveryman-detail-medias.html',
})
export class CityzenDeliverymanDetailMedias implements AfterViewInit {
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/deliveryman_detail_media_files?' + httpParams.toString()).subscribe({
      next: (response: any) => {
