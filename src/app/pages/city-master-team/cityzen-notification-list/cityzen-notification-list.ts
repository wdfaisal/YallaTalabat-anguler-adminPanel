...
@Component({
  selector: 'app-cityzen-notification-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-notification-list.html',
})
export class CityzenNotificationList {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  notification = new MatTableDataSource<CityzenNotificationListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/notification_list/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.results) {
          ...
          this.notification = new MatTableDataSource<CityzenNotificationListInterface>(mappedList);
