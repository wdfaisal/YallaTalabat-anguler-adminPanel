...
@Component({
  selector: 'app-cityzen-manage-restaurant-campaign',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-manage-restaurant-campaign.html',
})
export class CityzenManageRestaurantCampaign {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/cityzen/restaurant_campaign_deep_detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onSaveCampaign() {
    ...
    this.api.post_private('v1/cityzen/create_restaurant_campaign/' + this.util.getItem('_uid'), sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        this.util.onSuccess('ts_restaurant_campaign_added');
        this.router.navigate(['cityzen-team/campaign-section/restaurant-campaign']);
      }, error: (error: any) => {
        this.util.stop(spinnerRef);
        this.util.handleError(error, 'cityzen');
      }
  ...
  onUpdateCampaign() {
    ...
    console.log(sendData);
    const spinnerRef = this.util.start('ts_saving');
    this.api.patch_private('v1/cityzen/update_restaurant_campaign/' + this.id, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        this.util.onSuccess('ts_restaurant_campaign_updated');
        this.router.navigate(['cityzen-team/campaign-section/restaurant-campaign']);
      }, error: (error: any) => {
        this.util.stop(spinnerRef);
        this.util.handleError(error, 'cityzen');
  ...
  onReset() {
    ...
    const translations = this.campaignForm.get('translations') as FormArray;
    translations.clear();
