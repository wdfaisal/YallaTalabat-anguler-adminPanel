...
import { CityzenRestaurantFoodListLimitedInterface } from 'src/app/interfaces/cityzen.restaurant.foods.limited.detail.interface';
...
@Component({
  selector: 'app-cityzen-manage-food-campaign',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-manage-food-campaign.html',
})
export class CityzenManageFoodCampaign {
  ...
  foods: Observable<CityzenRestaurantFoodListLimitedInterface[]>;
  listOfFoods: CityzenRestaurantFoodListLimitedInterface[] = [];
  foodCtrl = new FormControl('');
  selectedFoodList: CityzenRestaurantFoodListLimitedInterface[] = [];
  ...
  getInfo() {
    ...
    this.api.get_private('v1/cityzen/food_campaign_deep_detail/' + this.id).subscribe({
      next: (response: any) => {
        ...
        if (values && values.id == this.id) {
          ...
          const mappedList = values.foods.map(
            (item: CityzenRestaurantFoodListLimitedInterface) => {
  ...
  onSaveCampaign() {
    ...
    this.api.post_private('v1/cityzen/create_food_campaign/' + this.util.getItem('_uid'), sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        this.util.onSuccess('ts_food_campaign_added');
        this.router.navigate(['cityzen-team/campaign-section/food-campaign']);
      }, error: (error: any) => {
        this.util.stop(spinnerRef);
        this.util.handleError(error, 'cityzen');
      }
  ...
  onUpdateCampaign() {
    ...
    console.log(sendData);
    const spinnerRef = this.util.start('ts_saving');
    this.api.patch_private('v1/cityzen/update_food_campaign/' + this.id, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  getFoodsByRestaurants(id: string) {
    const spinnerRef = this.util.start('ts_fetching');
    this.api.get_private('v1/cityzen/food_list_for_campaign/' + id).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  private _filterFoods(value: any): CityzenRestaurantFoodListLimitedInterface[] {
    console.log(value);
    let filterValue: string;
  ...
  onDelete(food: CityzenRestaurantFoodListLimitedInterface) {
  ...
  getTranslatedFoodName(item: CityzenRestaurantFoodListLimitedInterface): string {
    if (item.translations && Array.isArray(item.translations)) {
      const found = item.translations.find(t => t.code == this.util.appLocaleName());
      if (found && found.title) {
        return found.title;
