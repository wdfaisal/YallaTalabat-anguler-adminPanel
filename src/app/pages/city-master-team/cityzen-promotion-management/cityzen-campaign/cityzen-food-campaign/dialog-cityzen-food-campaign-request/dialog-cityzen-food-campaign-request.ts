...
import { CityzenFoodCampaignRequestListInterface } from 'src/app/interfaces/cityzen.food.campaign.request.list.interface';
...
@Component({
  selector: 'app-dialog-cityzen-food-campaign-request',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './dialog-cityzen-food-campaign-request.html',
})
export class DialogCityzenFoodCampaignRequest {

  @ViewChild('paginator', { read: MatPaginator }) paginator: MatPaginator;
  campaigns = new MatTableDataSource<CityzenFoodCampaignRequestListInterface>([]);
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogCityzenFoodCampaignRequest>,
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/food_campaign_request_list/' + this.id + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: CityzenFoodCampaignRequestListInterface) => {
              ...
              if (item && item.foods && item.foods?.id) {
                if (item.foods?.translations) {
                  ...
                  item.foods.displayShortDescription = translation?.shortDescription || item.foods.shortDescription;
                } else {
          ...
          this.campaigns = new MatTableDataSource<CityzenFoodCampaignRequestListInterface>(mappedList);
  ...
  acceptRequest(campaign: CityzenFoodCampaignRequestListInterface) {
    console.log('accept', campaign);
    const spinnerRef = this.util.start();
    this.api.get_private('v1/cityzen/accept_food_campaign_request/' + campaign.campaign + '/' + campaign.food).subscribe({
      next: (response: any) => {
        console.log(response);
  ...
  rejectRequest(campaign: CityzenFoodCampaignRequestListInterface) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result && result.action && result.action == "confirm") {
        ...
        const spinnerRef = this.util.start();
        this.api.delete_private('v1/cityzen/reject_food_campaign_request/' + campaign.id).subscribe({
          next: (response: any) => {
            console.log(response);
            this.util.stop(spinnerRef);
            this.getList();
  ...
  onRestaurantDetail(item: CityzenFoodCampaignRequestListInterface) {
    console.log(item);
