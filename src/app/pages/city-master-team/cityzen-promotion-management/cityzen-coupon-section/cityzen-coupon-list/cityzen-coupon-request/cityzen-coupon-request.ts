...
import { CityzenVendorCoupoRequestListInterface } from 'src/app/interfaces/cityzen.vendor.coupon.request.interface';
...
@Component({
  selector: 'app-cityzen-coupon-request',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-coupon-request.html',
})
export class CityzenCouponRequest {

  @ViewChild('paginator', { read: MatPaginator }) paginator: MatPaginator;
  coupons = new MatTableDataSource<CityzenVendorCoupoRequestListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/new_coupon_request/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.coupons) {
          const mappedList = response.coupons.map(
            (item: CityzenVendorCoupoRequestListInterface) => {
          ...
          this.coupons = new MatTableDataSource<CityzenVendorCoupoRequestListInterface>(mappedList);
  ...
  onEdit(coupon: CityzenVendorCoupoRequestListInterface) {
    console.log(coupon);
    this.router.navigate(['cityzen-team/u/edit-coupon/', coupon.id]);
  }
  ...
  onAccept(coupon: CityzenVendorCoupoRequestListInterface) {
    console.log('Accept');
    const spinnerRef = this.util.start();
    this.api.patch_private('v1/cityzen/update_meta_coupon/' + coupon.id, { status: 'live' }).subscribe({
      next: (response: any) => {
        console.log(response);
  ...
  onReject(coupon: CityzenVendorCoupoRequestListInterface) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result && result.action && result.action == "confirm") {
        ...
        const spinnerRef = this.util.start();
        this.api.delete_private('v1/cityzen/delete_coupon/' + coupon.id).subscribe({
          next: (response: any) => {
            console.log(response);
            this.util.stop(spinnerRef);
            this.getList();
  ...
  onRestaurantDetail(item: CityzenVendorCoupoRequestListInterface) {
    console.log(item);
