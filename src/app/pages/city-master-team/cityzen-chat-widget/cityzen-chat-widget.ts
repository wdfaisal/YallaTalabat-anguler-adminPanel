...
import { CityzenRegularChatConversionInterface } from 'src/app/interfaces/cityzen.regular.chat.conversion.interface';
import { CityzenRegularChatListInterface } from 'src/app/interfaces/cityzen.regular.chat.list.interface';
import { CityzenSupportChatConversionInterface } from 'src/app/interfaces/cityzen.support.chat.conversion.interface';
import { CityzenSupportChatInterface } from 'src/app/interfaces/cityzen.support.chat.interface';
...
@Component({
  selector: 'app-cityzen-chat-widget',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgScrollbarModule, NgIcon],
  templateUrl: './cityzen-chat-widget.html',
})
export class CityzenChatWidget {
  ...
  chatList: CityzenRegularChatListInterface[] = [];
  chatListDummy: CityzenRegularChatListInterface[] = [];
  supportChatList: CityzenSupportChatInterface[] = [];
  supportChatListDummy: CityzenSupportChatInterface[] = [];
  ...
  regularChatMessages: CityzenRegularChatConversionInterface[] = [];
  supportChatMessages: CityzenSupportChatConversionInterface[] = [];
  roomId: string = '';
  ...
  getSupportChatList() {
    ...
    this.api.get_private('v1/cityzen/support_chat_list/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
  ...
  loadMore() {
    ...
    if (this.currentChatType == 'regular') {
      if (this.serverResultsChat < this.totalResultsChat) {
        ...
        this.api.get_private('v1/cityzen/regular_chat_list/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
          next: (response: any) => {
  ...
  loadMoreRegularChatMessages() {
    ...
    this.api.get_private('v1/cityzen/regular_chat_messages/' + this.chatId + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
  ...
  loadMoreSupportChatMessages() {
    ...
    this.api.get_private('v1/cityzen/support_chat_messages/' + this.supportChatId + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
  ...
  onChatSelect(item: CityzenRegularChatListInterface) {
    console.log(item);
  ...
  onSupportChatSelect(item: CityzenSupportChatInterface) {
    console.log(item);
    this.chatInitialize = true;
    this.receiverImages = [];
    this.receiverName = '';
  ...
  sendMessage() {
    ...
    if (this.currentChatType == 'regular') {
      ...
      this.msg = '';
      this.api.post_private('v1/cityzen/chat_room/send_regular_message/', serverParam).subscribe({
        next: (response: any) => {
          console.log(response);
        }, error: (error: any) => {
          console.log(error);
      ...
      this.msg = '';
      this.api.post_private('v1/cityzen/chat_room/send_support_message/', serverParam).subscribe({
        next: (response: any) => {
          console.log(response);
        }, error: (error: any) => {
          console.log(error);
  ...
  chatName(chats: CityzenRegularChatListInterface): string {
