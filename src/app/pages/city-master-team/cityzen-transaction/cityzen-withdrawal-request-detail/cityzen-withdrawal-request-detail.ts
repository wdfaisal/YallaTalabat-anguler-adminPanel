...
import { DialogCityzenDeclineWithdrawalRequest } from './dialog-cityzen-decline-withdrawal-request/dialog-cityzen-decline-withdrawal-request';
import { DialogCityzenApproveWithdrawalRequest } from './dialog-cityzen-approve-withdrawal-request/dialog-cityzen-approve-withdrawal-request';
...
@Component({
  selector: 'app-cityzen-withdrawal-request-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-withdrawal-request-detail.html',
})
export class CityzenWithdrawalRequestDetail {

  credential: CityzenPayoutCredentialInterface[] = [];
  requestCredential: CityzenPayoutCredentialInterface[] = [];
  id: string = '';
  amount: number = 0;
  createdAt: string = '';
  from: string = '';
  ...
  getDetail() {
    ...
    this.api.get_private('v1/cityzen/withdrawal_request_detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onApproveRequest() {
    ...
    const dialogRef = this.dialog.open(DialogCityzenApproveWithdrawalRequest, {
  ...
  onDeclineRequest() {
    ...
    const dialogRef = this.dialog.open(DialogCityzenDeclineWithdrawalRequest, {
      data: { id: this.id, balance: this.walletBalance, amount: this.amount },
      disableClose: true
    });
