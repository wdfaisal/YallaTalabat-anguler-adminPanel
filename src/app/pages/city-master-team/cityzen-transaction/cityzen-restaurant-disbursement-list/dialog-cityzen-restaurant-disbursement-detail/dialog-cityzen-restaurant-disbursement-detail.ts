...
@Component({
  selector: 'app-dialog-cityzen-restaurant-disbursement-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-cityzen-restaurant-disbursement-detail.html',
})
export class DialogCityzenRestaurantDisbursementDetail {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogCityzenRestaurantDisbursementDetail>,
  ...
  getDetail() {
    ...
    this.api.get_private('v1/cityzen/restaurant_disbursement_detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onAccept() {
    console.log('Accept');
    this.isSubmit = true;
    this.api.get_private('v1/cityzen/accept_restaurant_disbursement/' + this.id).subscribe({
      next: (response: any) => {
        console.log(response);
