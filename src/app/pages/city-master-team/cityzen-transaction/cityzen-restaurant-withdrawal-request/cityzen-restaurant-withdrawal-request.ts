...
import { CityzenRestaurantWithdrawalRequestInterface } from 'src/app/interfaces/cityzen.restaurant.withdrawal.request.interface';
...
@Component({
  selector: 'app-cityzen-restaurant-withdrawal-request',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-restaurant-withdrawal-request.html',
})
export class CityzenRestaurantWithdrawalRequest {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  list = new MatTableDataSource<CityzenRestaurantWithdrawalRequestInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/restaurant_withdrawal_request/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: CityzenRestaurantWithdrawalRequestInterface) => {
          ...
          this.list = new MatTableDataSource<CityzenRestaurantWithdrawalRequestInterface>(mappedList);
  ...
  openDetail(detail: CityzenRestaurantWithdrawalRequestInterface) {
    console.log(detail);
    this.router.navigate(['/cityzen-team/u/withdrawal-request-detail/', detail.id]);
  }

  onRestaurantDetail(item: CityzenRestaurantWithdrawalRequestInterface) {
    console.log(item);
