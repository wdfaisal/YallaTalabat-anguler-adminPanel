...
import { CityzenCustomerDeliverymanReviewInterface, DeliverymanHashtag } from 'src/app/interfaces/cityzen.customer.deliveryman.review.interface';
import { CityzenCustomerFoodReviewInterface, FoodHashtag } from 'src/app/interfaces/cityzen.customer.food.review.interface';
import { CityzenCustomerRestaurantReviewInterface, RestaurantHashtag } from 'src/app/interfaces/cityzen.customer.restaurant.review.interface';
...
@Component({
  selector: 'app-cityzen-customer-detail-given-reviews',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-customer-detail-given-reviews.html',
})
export class CityzenCustomerDetailGivenReviews implements AfterViewInit {
  ...
  deliverymanReview: CityzenCustomerDeliverymanReviewInterface[] = [];
  restaurantReview: CityzenCustomerRestaurantReviewInterface[] = [];
  foodReview: CityzenCustomerFoodReviewInterface[] = [];
  ...
  getInitial() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_all_reviews?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.food && response.food.foodReviews) {
          const mappedList = response.food.foodReviews.map(
            (item: CityzenCustomerFoodReviewInterface) => {
        ...
        if (response && response.restaurant && response.restaurant.restaurantReviews) {
          const mappedList = response.restaurant.restaurantReviews.map(
            (item: CityzenCustomerRestaurantReviewInterface) => {
  ...
  getFoodReview() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_food_reviews?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedFood = true;
  ...
  getRestaurantReview() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_restaurant_reviews?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedRestaurant = true;
        if (response && response.results) {
  ...
  getDeliverymanReview() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_deliveryman_reviews?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedDeliveryman = true;
        if (response && response.results) {
  ...
  getTranslatedFoodHashtag(hash: FoodHashtag): string {
    const found = hash.translations?.find(t => t.code == this.util.appLocaleName());
    return found?.value || hash.displayName || hash.name;
  }

  getTranslatedRestaurantHashtag(hash: RestaurantHashtag): string {
    const found = hash.translations?.find(t => t.code == this.util.appLocaleName());
    return found?.value || hash.displayName || hash.name;
  }
