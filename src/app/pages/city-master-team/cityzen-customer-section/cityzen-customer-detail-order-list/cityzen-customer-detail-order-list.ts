...
@Component({
  selector: 'app-cityzen-customer-detail-order-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-customer-detail-order-list.html',
})
export class CityzenCustomerDetailOrderList implements AfterViewInit {

  @Input() userId!: string;
  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  orders = new MatTableDataSource<CityzenCustomerOrderListInterface>([]);
  displayedColumn = ['id', 'date', 'restaurant', 'amount', 'status', 'action'];
  pageSize: number = 5;
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_order_list?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.orders) {
          ...
          this.orders = new MatTableDataSource<CityzenCustomerOrderListInterface>(mappedList);
