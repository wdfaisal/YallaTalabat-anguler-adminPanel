...
import { CityzenCustomerFavouriteFoodInterface } from 'src/app/interfaces/cityzen.customer.favourite.food.interface';
import { CityzenCustomerFavouriteRestaurantInterface } from 'src/app/interfaces/cityzen.customer.favourite.restaurant.interface';
import { CityzenCustomerOrderListInterface } from 'src/app/interfaces/cityzen.customer.order.list.interface';
...
@Component({
  selector: 'app-cityzen-customer-detail-favourite-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-customer-detail-favourite-list.html',
})
export class CityzenCustomerDetailFavouriteList implements AfterViewInit {
  ...
  favouriteRestaurants = new MatTableDataSource<CityzenCustomerFavouriteRestaurantInterface>([]);
  favouriteFoods = new MatTableDataSource<CityzenCustomerFavouriteFoodInterface>([]);
  favouriteOrders = new MatTableDataSource<CityzenCustomerOrderListInterface>([]);
  ...
  getInitial() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_favourite_list?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.order && response.order.favouriteOrders) {
          const mappedList = response.order.favouriteOrders.map(
            (item: CityzenCustomerOrderListInterface) => {
          ...
          this.favouriteOrders = new MatTableDataSource<CityzenCustomerOrderListInterface>(mappedList);
        ...
        if (response && response.restaurant && response.restaurant.restaurantFavourite) {
          const mappedList = response.restaurant.restaurantFavourite.map(
            (item: CityzenCustomerFavouriteRestaurantInterface) => {
              if (item && item.restaurant && item.restaurant?.id) {
                if (item.restaurant?.translations) {
                  const translation = item.restaurant.translations.find((t) => t.code == this.util.appLocaleName());
                  item.restaurant.displayName = translation?.title || item.restaurant.name;
          ...
          this.favouriteRestaurants = new MatTableDataSource<CityzenCustomerFavouriteRestaurantInterface>(mappedList);
        ...
        if (response && response.food && response.food.foodFavourite) {
          const mappedList = response.food.foodFavourite.map(
            (item: CityzenCustomerFavouriteFoodInterface) => {
          ...
          this.favouriteFoods = new MatTableDataSource<CityzenCustomerFavouriteFoodInterface>(mappedList);
  ...
  getOrderList() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_favourite_orders?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedOrders = true;
  ...
  getRestaurantList() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_favourite_restaurants?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedRestaurants = true;
        if (response && response.results) {
  ...
  getFoodList() {
    ...
    this.api.get_private('v1/cityzen/customer_detail_favourite_foods?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedFoods = true;
        if (response && response.results) {
