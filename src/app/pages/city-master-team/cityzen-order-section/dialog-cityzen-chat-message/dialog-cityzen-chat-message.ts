...
@Component({
  selector: 'app-dialog-cityzen-chat-message',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgScrollbarModule],
  templateUrl: './dialog-cityzen-chat-message.html',
})
export class DialogCityzenChatMessage {
  ...
  roomID: string = '';
  chats: CityzenRegularChatConversionInterface[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogCityzenChatMessage>,
  ...
  getChatMessage() {
    ...
    this.isLoading = true;
    this.api.post_private('v1/cityzen/chat_room/fetch_messages/', param).subscribe({
      next: (response: any) => {
  ...
  loadMoreMessages() {
    if (this.serverResultChatMessage < this.totalResultChatMessage) {
      ...
      this.api.get_private('v1/cityzen/regular_chat_messages/' + this.roomID + '?' + httpParams.toString()).subscribe({
        next: (response: any) => {
          console.log(response);
