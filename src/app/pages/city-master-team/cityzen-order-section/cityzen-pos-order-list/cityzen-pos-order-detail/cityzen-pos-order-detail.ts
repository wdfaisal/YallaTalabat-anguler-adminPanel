...
@Component({
  selector: 'app-cityzen-pos-order-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-pos-order-detail.html',
})
export class CityzenPosOrderDetail {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/cityzen/cityzen_pos_order_detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onPrint() {
    this.router.navigate(['cityzen-team/u/pos-order-invoice', this.id]);
  }
