...
import { CityzenUserComplaintsListInterface } from 'src/app/interfaces/cityzen.user.complaints.list.interface';
...
@Component({
  selector: 'app-cityzen-order-complaint-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-order-complaint-list.html',
})
export class CityzenOrderComplaintList {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  complaints = new MatTableDataSource<CityzenUserComplaintsListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/orders_complaint_list/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.complaints) {
          const mappedList = response.complaints.map(
            (item: CityzenUserComplaintsListInterface) => {
          ...
          this.complaints = new MatTableDataSource<CityzenUserComplaintsListInterface>(mappedList);
  ...
  onUserInfo(item: CityzenUserComplaintsListInterface) {
    console.log(item);
