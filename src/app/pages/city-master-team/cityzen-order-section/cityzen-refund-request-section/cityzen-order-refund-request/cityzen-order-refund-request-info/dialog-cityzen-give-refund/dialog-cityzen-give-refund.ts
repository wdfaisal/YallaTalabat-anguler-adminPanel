...
@Component({
  selector: 'app-dialog-cityzen-give-refund',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-cityzen-give-refund.html',
})
export class DialogCityzenGiveRefund {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogCityzenGiveRefund>,
    ...
    if (this.data && this.data.requestId !== null && this.data.requestId != '') {
      ...
      this.refundForm.controls['orderId'].setValue(this.data.orderId);
  ...
  onRefundFromMerchant() {
    ...
    this.isSubmit = true;
    this.api.post_private('v1/cityzen/refund_request/refundFromMerchant', { requestId: this.requestId }).subscribe({
      next: (response: any) => {
  ...
  onSubmit() {
    ...
    if (this.refundForm.valid) {
      ...
      if (refundedAmount <= this.grandTotal) {
        console.log('All OK');
        this.isSubmit = true;
        this.api.post_private('v1/cityzen/refund_request/approve', this.refundForm.getRawValue()).subscribe({
          next: (response: any) => {
            console.log(response);
            if (response && response.success == true) {
              this.isSubmit = false
