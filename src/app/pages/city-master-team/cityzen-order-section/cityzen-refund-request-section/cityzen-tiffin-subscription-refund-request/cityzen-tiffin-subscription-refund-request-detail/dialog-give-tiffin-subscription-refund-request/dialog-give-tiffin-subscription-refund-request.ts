...
@Component({
  selector: 'app-dialog-give-tiffin-subscription-refund-request',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-give-tiffin-subscription-refund-request.html',
})
export class DialogGiveTiffinSubscriptionRefundRequest {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogGiveTiffinSubscriptionRefundRequest>,
  ...
  onRefundFromMerchant() {
    ...
    this.isSubmit = true;
    this.api.post_private('v1/cityzen/tiffin_subscription_refund_request/refundFromMerchant', { requestId: this.requestId }).subscribe({
      next: (response: any) => {
  ...
  onSubmit() {
    ...
    if (this.refundForm.valid) {
      ...
      if (refundedAmount <= this.grandTotal) {
        console.log('All OK');
        this.isSubmit = true;
        this.api.post_private('v1/cityzen/tiffin_subscription_refund_request/approve', this.refundForm.getRawValue()).subscribe({
          next: (response: any) => {
            console.log(response);
            if (response && response.success == true) {
              this.isSubmit = false
