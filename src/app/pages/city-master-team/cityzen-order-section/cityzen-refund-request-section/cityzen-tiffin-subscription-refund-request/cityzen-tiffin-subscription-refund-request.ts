...
import { CityzenTiffinSubscriptionRefundRequestListInterface } from 'src/app/interfaces/cityzen.tiffin.subscription.refund.request.interface';
...
@Component({
  selector: 'app-cityzen-tiffin-subscription-refund-request',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-tiffin-subscription-refund-request.html',
})
export class CityzenTiffinSubscriptionRefundRequest {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  requests = new MatTableDataSource<CityzenTiffinSubscriptionRefundRequestListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/tiffin_subscription_refund_request_list/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.request) {
          const mappedList = response.request.map(
            (item: CityzenTiffinSubscriptionRefundRequestListInterface) => {
          ...
          this.requests = new MatTableDataSource<CityzenTiffinSubscriptionRefundRequestListInterface>(mappedList);
  ...
  onUserInfo(item: CityzenTiffinSubscriptionRefundRequestListInterface) {
    console.log(item);
  ...
  onRestaurantDetail(item: CityzenTiffinSubscriptionRefundRequestListInterface) {
    console.log(item);
