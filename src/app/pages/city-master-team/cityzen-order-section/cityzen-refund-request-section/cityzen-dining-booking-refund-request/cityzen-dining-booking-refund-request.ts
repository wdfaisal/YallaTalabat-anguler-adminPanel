...
import { CityzenDiningBookingRefundRequestListInterface } from 'src/app/interfaces/cityzen.dining.booking.refund.request.list.interface';
...
@Component({
  selector: 'app-cityzen-dining-booking-refund-request',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-dining-booking-refund-request.html',
})
export class CityzenDiningBookingRefundRequest {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  requests = new MatTableDataSource<CityzenDiningBookingRefundRequestListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/dining_booking_refund_request_list/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.request) {
          const mappedList = response.request.map(
            (item: CityzenDiningBookingRefundRequestListInterface) => {
          ...
          this.requests = new MatTableDataSource<CityzenDiningBookingRefundRequestListInterface>(mappedList);
  ...
  onUserInfo(item: CityzenDiningBookingRefundRequestListInterface) {
    console.log(item);
  ...
  onRestaurantDetail(item: CityzenDiningBookingRefundRequestListInterface) {
    console.log(item);
