...
@Component({
  selector: 'app-cityzen-order-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-order-detail.html',
})
export class CityzenOrderDetail {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/cityzen/cityzen_order_detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onOrderPrint() {
    console.log('print');
    this.router.navigate(['cityzen-team/u/order-invoice', this.id]);
  }
