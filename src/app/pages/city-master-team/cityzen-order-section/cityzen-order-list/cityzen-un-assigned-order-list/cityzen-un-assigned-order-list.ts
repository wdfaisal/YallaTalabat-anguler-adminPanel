...
import { CityzenDialogAssignDriverOrder } from './cityzen-dialog-assign-driver-order/cityzen-dialog-assign-driver-order';
...
@Component({
  selector: 'app-cityzen-un-assigned-order-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cityzen-un-assigned-order-list.html',
})
export class CityzenUnAssignedOrderList {
  ...
  getList() {
    ...
    this.api.get_private('v1/cityzen/un_assinged_order_list/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
  ...
  assignDriver(item: CityzenOrdersListInterface) {
    console.log(item);
    this.dialog.open(CityzenDialogAssignDriverOrder, {
