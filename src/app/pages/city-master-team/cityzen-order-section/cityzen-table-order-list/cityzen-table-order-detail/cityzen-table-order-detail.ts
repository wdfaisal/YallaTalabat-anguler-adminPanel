...
@Component({
  selector: 'app-cityzen-table-order-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './cityzen-table-order-detail.html',
})
export class CityzenTableOrderDetail {
  ...
  getOrderDetail() {
    ...
    this.api.get_private('v1/cityzen/cityzen_table_order_detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onPrint() {
    this.router.navigate(['cityzen-team/u/table-order-invoice', this.id]);
  }
