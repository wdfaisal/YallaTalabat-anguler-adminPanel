...
@Component({
  selector: 'app-support-team-customer-tiffin-subscription-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './support-team-customer-tiffin-subscription-detail.html',
})
export class SupportTeamCustomerTiffinSubscriptionDetail {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/support_team/purchase_detail/' + this.id).subscribe({
      next: (response: any) => {
        ...
        if (response && response.success == true) {
          ...
          if (response && response.success == true && response.details) {
            ...
            if (purchaseInfo && purchaseInfo.userInfo && purchaseInfo.userInfo.id) {
              ...
              this.userContanct = `+${purchaseInfo.userInfo.countryCode} ${purchaseInfo.userInfo.mobile}`;
              this.userRole = purchaseInfo.userInfo.role;
              this.userCover = purchaseInfo.userInfo.image;
              this.userEmail = purchaseInfo.userInfo.email;
            }
