...
@Component({
  selector: 'app-support-team-profile-setting',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './support-team-profile-setting.html',
})
export class SupportTeamProfileSetting {
  ...
  getDetail() {
    ...
    this.api.get_private('v1/support_team/support_team_profile/' + this.util.getItem('_uid')).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
    ...
    if (this.form.valid) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result && result.action && result.action == "confirm") {
          ...
          this.api.patch_private('v1/support_team/update_support_team/' + this.util.getItem('_uid'), this.form.getRawValue()).subscribe({
            next: (response: any) => {
              console.log(response);
              this.util.stop(spinnerRef);
                ...
                if (response.reload == true) {
                  this.api.post_public('v1/auth/logout_web', {}).subscribe({
                    next: (response: any) => {
                      console.log(response);
                      this.util.clearItem();
                      this.router.navigate(['/authentication/support']);
                    }, error: (error: any) => {
                      console.log(error);
                      this.util.clearItem();
                      this.router.navigate(['/authentication/support']);
    ...
    if (this.passwordForm.valid) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result && result.action && result.action == "confirm") {
          console.log('confirmed');
          const spinnerRef = this.util.start('ts_updating');
          this.api.patch_private('v1/support_team/update_support_team_password/' + this.util.getItem('_uid'), { "password": this.passwordForm.controls['password'].value }).subscribe({
            next: (response: any) => {
              console.log(response);
              this.util.stop(spinnerRef);
              this.api.post_public('v1/auth/logout_web', {}).subscribe({
