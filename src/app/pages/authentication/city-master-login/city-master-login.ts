...
@Component({
  selector: 'app-city-master-login',
  imports: [RouterModule, MaterialModule, FormsModule, ReactiveFormsModule, BrandingComponent],
  templateUrl: './city-master-login.html',
})
export class CityMasterLogin {
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      try {
        this.isSubmit = true;
        this.api.post_public('v1/auth/cityzen/login', this.form.value).subscribe({
          next: (response: any) => {
            console.log(response);
            this.isSubmit = false;
            if (response && response.user && response.user.id) {
              this.util.setItem('_uid', response.user.id);
              this.util.setItem('_cityMaster', response.user.city);
              this.util.setItem('_authRole', 'cityMaster');
              ...
              this.router.navigate(['/cityzen-team']);
            } else {
