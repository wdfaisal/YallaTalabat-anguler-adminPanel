...
import { AdminFoodListInterface } from 'src/app/interfaces/admin.foods.list.interface';
...
export class Foods {
  ...
  foods = new MatTableDataSource<AdminFoodListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/foods/getAll?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AdminFoodListInterface) => {
          ...
          this.foods = new MatTableDataSource<AdminFoodListInterface>(mappedList);
  ...
  onAddFood() {
    this.router.navigate(['admin/foods/add-food']);
  }
  ...
  onStockChange(event: MatSlideToggleChange, food: AdminFoodListInterface) {
    console.log(event);
    console.log(food);
  ...
  onEdit(food: AdminFoodListInterface) {
    this.router.navigate(['admin/foods/edit-food', food.id]);
  }

  onDelete(food: AdminFoodListInterface) {
  ...
  onStatusChange(event: MatSelectChange, food: AdminFoodListInterface) {
    console.log(event);
  ...
  onRestaurantDetail(item: AdminFoodListInterface) {
    console.log(item);
  ...
  onFoodDetail(food: AdminFoodListInterface) {
    this.router.navigate(['admin/foods/food-detail', food.id]);
  }
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/foods/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
            const fileName = exportOption == 'excel' ? 'Foods.xlsx' : 'Foods.csv';
  ...
  importCollection() {
    this.router.navigate(['admin/import-export-management/import-collection/', 'foods']);
  }
