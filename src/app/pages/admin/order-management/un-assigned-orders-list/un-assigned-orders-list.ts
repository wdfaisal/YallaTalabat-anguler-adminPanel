...
import { DialogAssignDriverOrder } from './dialog-assign-driver-order/dialog-assign-driver-order';
...
@Component({
  selector: 'app-un-assigned-orders-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './un-assigned-orders-list.html',
})
export class UnAssignedOrdersList {
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/orders/getUnAssignedOrders?' + httpParams.toString()).subscribe({
      next: (response: any) => {
  ...
  assignDriver(item: OrdersListInterface) {
    console.log(item);
    this.dialog.open(DialogAssignDriverOrder, {
      data: { value: { id: item.id, restaurant: item.restaurant.id } },
      disableClose: true,
      autoFocus: false,
      restoreFocus: false,
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/unassigned_orders/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
