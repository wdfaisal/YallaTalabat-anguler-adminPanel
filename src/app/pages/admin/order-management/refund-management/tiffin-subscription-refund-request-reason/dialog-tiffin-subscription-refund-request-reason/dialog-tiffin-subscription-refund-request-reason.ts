...
@Component({
  selector: 'app-dialog-tiffin-subscription-refund-request-reason',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-tiffin-subscription-refund-request-reason.html',
})
export class DialogTiffinSubscriptionRefundRequestReason {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogTiffinSubscriptionRefundRequestReason>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/tiffin_subscription_refund_request_reason/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/tiffin_subscription_refund_request_reason/update/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
