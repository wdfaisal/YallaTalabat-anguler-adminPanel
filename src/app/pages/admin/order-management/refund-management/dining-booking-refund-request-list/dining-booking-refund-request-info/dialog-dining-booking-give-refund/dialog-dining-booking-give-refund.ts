...
@Component({
  selector: 'app-dialog-dining-booking-give-refund',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-dining-booking-give-refund.html',
})
export class DialogDiningBookingGiveRefund {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogDiningBookingGiveRefund>,
  ...
  onRefundFromMerchant() {
    ...
    this.isSubmit = true;
    this.api.post_private('v1/admin/dining_booking_refund_request/refundFromMerchant', { requestId: this.requestId }).subscribe({
      next: (response: any) => {
  ...
  onSubmit() {
    ...
    if (this.refundForm.valid) {
      ...
      if (refundedAmount <= this.grandTotal) {
        console.log('All OK');
        this.isSubmit = true;
        this.api.post_private('v1/admin/dining_booking_refund_request/approve', this.refundForm.getRawValue()).subscribe({
          next: (response: any) => {
            console.log(response);
            if (response && response.success == true) {
              this.isSubmit = false
