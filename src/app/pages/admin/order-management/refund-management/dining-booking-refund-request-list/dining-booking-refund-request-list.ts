...
import { AdminDiningBookingRefundRequestListInterface } from 'src/app/interfaces/admin.dining.booking.refund.request.list.interface';
...
@Component({
  selector: 'app-dining-booking-refund-request-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dining-booking-refund-request-list.html',
})
export class DiningBookingRefundRequestList {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  requests = new MatTableDataSource<AdminDiningBookingRefundRequestListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/dining_booking_refund_request/active?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.request) {
          const mappedList = response.request.map(
            (item: AdminDiningBookingRefundRequestListInterface) => {
          ...
          this.requests = new MatTableDataSource<AdminDiningBookingRefundRequestListInterface>(mappedList);
  ...
  onRequestDetail(bookingId: string) {
    console.log(`Booking Id -> ${bookingId}`);
    this.router.navigate(['admin/order-management/dining-booking-refund-request', bookingId]);
  }

  onUserInfo(item: AdminDiningBookingRefundRequestListInterface) {
    console.log(item);
  ...
  onRestaurantDetail(item: AdminDiningBookingRefundRequestListInterface) {
    console.log(item);
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/dining_booking_refund_request/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
            const fileName = exportOption == 'excel' ? 'DiningBookingRefund.xlsx' : 'DiningBookingRefund.csv';
            this.api.download_export_file(blob, fileName);
          } else {
            const fileName = 'diningbookingrefundrequests.json';
            this.api.download_export_file(blob, fileName);
          }
  ...
  importCollection() {
    this.router.navigate(['admin/import-export-management/import-collection/', 'dining_refund_request']);
  }
