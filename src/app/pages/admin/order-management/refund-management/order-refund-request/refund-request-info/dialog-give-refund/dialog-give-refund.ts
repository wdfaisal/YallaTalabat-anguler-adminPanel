...
@Component({
  selector: 'app-dialog-give-refund',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-give-refund.html',
})
export class DialogGiveRefund {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogGiveRefund>,
  ...
  onRefundFromMerchant() {
    ...
    this.isSubmit = true;
    this.api.post_private('v1/admin/refund_request/refundFromMerchant', { requestId: this.requestId }).subscribe({
      next: (response: any) => {
  ...
  onSubmit() {
    ...
    if (this.refundForm.valid) {
      ...
      if (refundedAmount <= this.grandTotal) {
        console.log('All OK');
        this.isSubmit = true;
        this.api.post_private('v1/admin/refund_request/approve', this.refundForm.getRawValue()).subscribe({
          next: (response: any) => {
            console.log(response);
            if (response && response.success == true) {
              this.isSubmit = false
