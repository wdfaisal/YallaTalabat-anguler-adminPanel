...
import { DialogCancelRefund } from './dialog-cancel-refund/dialog-cancel-refund';
import { DialogGiveRefund } from './dialog-give-refund/dialog-give-refund';
...
@Component({
  selector: 'app-refund-request-info',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './refund-request-info.html',
})
export class RefundRequestInfo {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/admin/refund_request/info/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onOrderPrint() {
    this.router.navigate(['admin/order-management/order-invoice', this.orderId]);
  }
  ...
  onCancelRefundRequest() {
    const dialogRef = this.dialog.open(DialogCancelRefund, {
  ...
  onGiveRefund() {
    const dialogRef = this.dialog.open(DialogGiveRefund, {
