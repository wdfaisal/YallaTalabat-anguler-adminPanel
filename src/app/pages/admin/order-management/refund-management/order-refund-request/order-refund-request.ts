...
import { RefundRequestListInterface } from 'src/app/interfaces/refund.request.list.interface';
...
@Component({
  selector: 'app-order-refund-request',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './order-refund-request.html',
})
export class OrderRefundRequest {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  requests = new MatTableDataSource<RefundRequestListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/refund_request/active?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.request) {
          const mappedList = response.request.map(
            (item: RefundRequestListInterface) => {
          ...
          this.requests = new MatTableDataSource<RefundRequestListInterface>(mappedList);
  ...
  onUserInfo(item: RefundRequestListInterface) {
    console.log(item);
  ...
  onRestaurantDetail(item: RefundRequestListInterface) {
    console.log(item);
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/refund_request/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
            const fileName = exportOption == 'excel' ? 'OrderRefundRequest.xlsx' : 'OrderRefundRequest.csv';
            this.api.download_export_file(blob, fileName);
          } else {
            const fileName = 'refundrequests.json';
            this.api.download_export_file(blob, fileName);
          }
  ...
  importCollection() {
    this.router.navigate(['admin/import-export-management/import-collection/', 'order_refund_request']);
  }
