...
@Component({
  selector: 'app-dialog-refund-request-reason',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-refund-request-reason.html',
})
export class DialogRefundRequestReason {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogRefundRequestReason>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/refund_request_reason/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/refund_request_reason/update/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
