...
import { AdminRestaurantListForTiffinPackagesInterface, RestaurantLocality } from 'src/app/interfaces/admin.restaurant.list.for.tiffin.packages.interface';
import { AdminFoodListForTiffinPackageInterface } from 'src/app/interfaces/admin.food.list.for.tiffin.package.interface';
...
import { AdminWeekListForSubscriptionInterface } from 'src/app/interfaces/admin.week.list.for.subscription.interface';
import { DialogAdminTiffinSubscriptionNotice } from './dialog-admin-tiffin-subscription-notice/dialog-admin-tiffin-subscription-notice';
import { DialogAdminTiffinSubscriptionTimeslot } from './dialog-admin-tiffin-subscription-timeslot/dialog-admin-tiffin-subscription-timeslot';
...
@Component({
  selector: 'app-manage-admin-tiffin-subscription-package',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './manage-admin-tiffin-subscription-package.html',
})
export class ManageAdminTiffinSubscriptionPackage {
  ...
  languages: any[] = [];
  translations: any[] = [];
  foods: Observable<AdminFoodListForTiffinPackageInterface[]>;
  listOfFoods: AdminFoodListForTiffinPackageInterface[] = [];
  foodCtrl = new FormControl('');
  foodInPackage: AdminFoodListForTiffinPackageInterface[] = [];
  foodIdInPackage: string[] = [];
  noticeArray: AdminSubscriptionTiffinNoticeInterface[] = [];
  weekList: Observable<AdminWeekListForSubscriptionInterface[]>;
  listOfWeek: AdminWeekListForSubscriptionInterface[] = [];
  weekCtrl = new FormControl('');
  slotList: AdminSlotListForSubscriptionTiffinInterface[] = [];
  cities: Observable<AdminCitiesListLimitedInterface[]>;
  listOfCities: AdminCitiesListLimitedInterface[] = [];
  cityCtrl = new FormControl('');
  restaurants: Observable<AdminRestaurantListForTiffinPackagesInterface[]>;
  listOfRestaurants: AdminRestaurantListForTiffinPackagesInterface[] = [];
  ...
  getInfo() {
    ...
    this.api.get_private('v1/admin/tiffin_packages/details/' + this.id + '/' + this.packageForm.controls['restaurant'].value).subscribe({
      next: (response: any) => {
        ...
        if (response && response.info && response.info.id == this.id) {
          ...
          if (detail && detail.notice && detail.notice instanceof Array) {
            ...
            const mappedList = detail.notice.map(
              (item: AdminSubscriptionTiffinNoticeInterface) => {
          ...
          if (response && response.food) {
            const mappedList = response.food.map(
              (item: AdminFoodListForTiffinPackageInterface) => {
                if (item.translations) {
                  const translation = item.translations.find((t) => t.code == this.util.appLocaleName());
  ...
  private _filterWeek(value: string): AdminWeekListForSubscriptionInterface[] {
    const filterValue = value.toLowerCase();
  ...
  private _filterFoods(value: any): AdminFoodListForTiffinPackageInterface[] {
    console.log(value);
  ...
  onCitySelect(event: MatAutocompleteSelectedEvent) {
    ...
    if (event && event.option && event.option.value) {
      ...
      this.packageForm.controls['foods'].setValue('');
  ...
  getRestaurantByCity(id: string) {
    ...
    const spinnerRef = this.util.start('ts_fetching');
    this.api.get_private('v1/admin/restaurant/getRestaurantByCityIdForTiffinPackages/' + id).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        if (response && response.restaurants && response.restaurants) {
          const mappedList = response.restaurants.map(
            (item: AdminRestaurantListForTiffinPackagesInterface) => {
  ...
  getFoodsByRestaurants(id: string) {
    const spinnerRef = this.util.start('ts_fetching');
    this.api.get_private('v1/admin/tiffin_packages/foods/' + id).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  private _filterRestaurant(value: any): AdminRestaurantListForTiffinPackagesInterface[] {
    let filterValue: string;
    if (value && value.name) {
  ...
  getTranslateRestaurantName(item: AdminRestaurantListForTiffinPackagesInterface): string {
  ...
  displayFoodName(food: AdminFoodListForTiffinPackageInterface) {
    return food && food.displayName ? food.displayName : '';
  }
  ...
  onDeleteFoodFromPackage(food: AdminFoodListForTiffinPackageInterface) {
    console.log(food);
  ...
  savePackage() {
    ...
    this.api.post_private('v1/admin/tiffin_packages/create', sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        this.util.onSuccess('ts_package_added');
        this.router.navigate(['admin/order-management/subscription-tiffin-package']);
      }, error: (error: any) => {
        this.util.stop(spinnerRef);
        this.util.handleError(error, 'admin');
      }
  ...
  updatePackage() {
    ...
    console.log(sendData);
    const spinnerRef = this.util.start('ts_saving');
    this.api.patch_private('v1/admin/tiffin_packages/update/' + this.id, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  onReset() {
    ...
    this.packageForm.get('totalOrder')?.disable();
