...
@Component({
  selector: 'app-dialog-tiffin-subscription-cancellation-reason',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-tiffin-subscription-cancellation-reason.html',
})
export class DialogTiffinSubscriptionCancellationReason {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogTiffinSubscriptionCancellationReason>,
    ...
    if (this.data && this.data.action == 'edit') {
      ...
      this.reasonForm.controls['type'].setValue(values && values.type ? values.type : '');
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/tiffin_subscription_cancel_reason/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/tiffin_subscription_cancel_reason/update/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
