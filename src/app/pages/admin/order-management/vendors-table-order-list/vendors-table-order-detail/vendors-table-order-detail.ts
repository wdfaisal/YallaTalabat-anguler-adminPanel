...
@Component({
  selector: 'app-vendors-table-order-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './vendors-table-order-detail.html',
})
export class VendorsTableOrderDetail {
  ...
  getOrderDetail() {
    ...
    this.api.get_private('v1/admin/tableOrders/detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onPrint() {
    this.router.navigate(['admin/order-management/vendors-table-order-invoice', this.id]);
  }
