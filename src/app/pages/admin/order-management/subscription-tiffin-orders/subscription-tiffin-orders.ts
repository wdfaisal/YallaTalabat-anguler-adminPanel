...
import { AdminTiffinSubscriptionOrderListInterface } from 'src/app/interfaces/admin.tiffin.subscription.order.list.interface';
...
@Component({
  selector: 'app-subscription-tiffin-orders',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './subscription-tiffin-orders.html',
})
export class SubscriptionTiffinOrders {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  orders = new MatTableDataSource<AdminTiffinSubscriptionOrderListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/orders/getSubscriptionOrders?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.orders) {
          const mappedList = response.orders.map(
            (item: AdminTiffinSubscriptionOrderListInterface) => {
          ...
          this.orders = new MatTableDataSource<AdminTiffinSubscriptionOrderListInterface>(mappedList);
  ...
  onUserInfo(item: AdminTiffinSubscriptionOrderListInterface) {
    console.log(item);
  ...
  onRestaurantDetail(item: AdminTiffinSubscriptionOrderListInterface) {
    console.log(item);
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/subscription_orders/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
