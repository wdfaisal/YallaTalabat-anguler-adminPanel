...
@Component({
  selector: 'app-dialog-complaints-reason',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-complaints-reason.html',
})
export class DialogComplaintsReason {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogComplaintsReason>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/complaints_reason/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/complaints_reason/update/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
