...
import { AdminVendorDiningBookingListInterface } from 'src/app/interfaces/admin.vendor.dining.booking.list.interface';
...
@Component({
  selector: 'app-restaurant-detail-dining-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './restaurant-detail-dining-list.html',
})
export class RestaurantDetailDiningList implements AfterViewInit {

  @Input() restaurantId!: string;
  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  bookings = new MatTableDataSource<AdminVendorDiningBookingListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/vendor_detail/booking_list?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.bookings) {
          const mappedList = response.bookings.map(
            (item: AdminVendorDiningBookingListInterface) => {
          ...
          this.bookings = new MatTableDataSource<AdminVendorDiningBookingListInterface>(mappedList);
  ...
  onUserInfo(item: AdminVendorDiningBookingListInterface) {
    console.log(item);
