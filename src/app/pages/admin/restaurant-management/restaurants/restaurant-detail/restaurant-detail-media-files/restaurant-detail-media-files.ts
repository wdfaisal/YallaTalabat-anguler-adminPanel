...
@Component({
  selector: 'app-restaurant-detail-media-files',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgScrollbarModule],
  templateUrl: './restaurant-detail-media-files.html',
})
export class RestaurantDetailMediaFiles implements AfterViewInit {
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/vendor_detail/media_files?' + httpParams.toString()).subscribe({
      next: (response: any) => {
