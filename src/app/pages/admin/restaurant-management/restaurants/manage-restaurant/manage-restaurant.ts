...
import { DialogDeliveryTimePicker } from './dialog-delivery-time-picker/dialog-delivery-time-picker';
...
import { AdminCityListForRestaurantInterface } from 'src/app/interfaces/admin.cities.list.for.restaurant.interface';
import { AdminCuisineListForRestaurantInterface } from 'src/app/interfaces/admin.cuisine.list.for.restaurant.interface';
import { AdminSubscriptionListForRestaurantInterface } from 'src/app/interfaces/admin.subscription.list.for.restaurant.interface';
...
import { AdminFacilitiesListForNewRestaurantInterface } from 'src/app/interfaces/admin.facilities.list.for.new.restaurant.interface';
...
export class ManageRestaurant {
  ...
  languages: any[] = [];
  translations: any[] = [];
  cuisineList: Observable<AdminCuisineListForRestaurantInterface[]>;
  listOfCuisine: AdminCuisineListForRestaurantInterface[] = [];
  ...
  facilitiesList: Observable<AdminFacilitiesListForNewRestaurantInterface[]>;
  listOfFacilities: AdminFacilitiesListForNewRestaurantInterface[] = [];
  facilitiesCtrl = new FormControl('');
  cities: Observable<AdminCityListForRestaurantInterface[]>;
  listOfCities: AdminCityListForRestaurantInterface[] = [];
  ...
  tempLocality: string = '';
  subscriptions: Observable<AdminSubscriptionListForRestaurantInterface[]>;
  listOfSubscriptions: AdminSubscriptionListForRestaurantInterface[] = [];
  ...
  getInfo() {
    ...
    this.api.get_private('v1/admin/restaurant/getById/' + this.id).subscribe({
      next: (response: any) => {
        ...
        if (response && response.cuisine && response.cuisine.length) {
          const mappedList = response.cuisine.map(
            (item: AdminCuisineListForRestaurantInterface) => {
        ...
        if (response && response.types && response.types.length) {
          const mappedList = response.types.map(
            (item: AdminRestaurantTypeListForRestaurantInterface) => {
              if (item.translations) {
                const translation = item.translations.find((t) => t.code == this.util.appLocaleName());
                item.displayName = translation?.value || item.name;
              } else {
        ...
        if (response && response.facilities && response.facilities.length) {
          const mappedList = response.facilities.map(
            (item: AdminFacilitiesListForNewRestaurantInterface) => {
              if (item.translations) {
                const translation = item.translations.find((t) => t.code == this.util.appLocaleName());
                item.displayName = translation?.value || item.name;
              } else {
        ...
        if (response && response.subscription && response.subscription.length) {
          const mappedList = response.subscription.map(
            (item: AdminSubscriptionListForRestaurantInterface) => {
              if (item.translations) {
                const translation = item.translations.find((t) => t.code == this.util.appLocaleName());
  ...
  getBasicData() {
    const spinnerRef = this.util.start('ts_fetching');
    this.api.get_private('v1/admin/restaurant/getBasicDataForNewRestaurant').subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        if (response && response.cities && response.cities.length) {
  ...
  private _filterCities(value: string): AdminCityListForRestaurantInterface[] {
  ...
  private _filterCuisine(value: string): AdminCuisineListForRestaurantInterface[] {
    const filterValue = value.toLowerCase();
  ...
  private _filterRestaurantType(value: string): AdminRestaurantTypeListForRestaurantInterface[] {
    const filterValue = value.toLowerCase();
  ...
  private _filterRestaurantFacilities(value: string): AdminFacilitiesListForNewRestaurantInterface[] {
    const filterValue = value.toLowerCase();
  ...
  private _filterSubscription(value: string): AdminSubscriptionListForRestaurantInterface[] {
    const filterValue = value.toLowerCase();
  ...
  saveRestaurant() {
    ...
    const localityValue = this.restaurantForm.value?.locality;
    ...
    this.api.post_private('v1/admin/restaurant/save', sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        this.util.onSuccess('ts_restaurant_added');
        this.router.navigate(['admin/restaurant-management/restaurants']);
      }, error: (error: any) => {
        this.util.stop(spinnerRef);
        this.util.handleError(error, 'admin');
      }
  ...
  updateRestaurant() {
    ...
    this.api.patch_private('v1/admin/restaurant/update/' + this.id, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  onSubscriptionSelect(event: MatAutocompleteSelectedEvent) {
    ...
    if (event && event.option && event.option.value) {
      ...
      if (selectedSubscriptions && selectedSubscriptions.length > 0) {
        ...
        console.log(customCategory, ownDriver, pos, promote, multiOutlet, preBooking, tableOrder, ownKitchen);
        this.restaurantForm.controls['customCategory'].setValue(customCategory);
        this.restaurantForm.controls['ownDriver'].setValue(ownDriver);
  ...
  getTranslatedCityName(item: AdminCityListForRestaurantInterface): string {
  ...
  getTranslatedSubscriptionName(item: AdminSubscriptionListForRestaurantInterface): string {
    if (item.translations && Array.isArray(item.translations)) {
      const found = item.translations.find(t => t.code == this.util.appLocaleName());
