...
import { AdminHiddenRestaurantListInterface } from 'src/app/interfaces/admin.hidden.restaurant.list.interface';
...
@Component({
  selector: 'app-hidden-restaurant-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './hidden-restaurant-list.html',
})
export class HiddenRestaurantList {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  reports = new MatTableDataSource<AdminHiddenRestaurantListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/hide/restaurant?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.hidden) {
          const mappedList = response.hidden.map(
            (item: AdminHiddenRestaurantListInterface) => {
          ...
          this.reports = new MatTableDataSource<AdminHiddenRestaurantListInterface>(mappedList);
  ...
  onUserInfo(item: AdminHiddenRestaurantListInterface) {
    console.log(item);
  ...
  onRestaurantDetail(item: AdminHiddenRestaurantListInterface) {
    console.log(item);
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/hidden_restaurants/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
            const fileName = exportOption == 'excel' ? 'HiddenRestaurants.xlsx' : 'HiddenRestaurants.csv';
            this.api.download_export_file(blob, fileName);
          } else {
            const fileName = 'hiderestaurants.json';
            this.api.download_export_file(blob, fileName);
          }
  ...
  importCollection() {
    this.router.navigate(['admin/import-export-management/import-collection/', 'hidden_restaurants']);
  }
