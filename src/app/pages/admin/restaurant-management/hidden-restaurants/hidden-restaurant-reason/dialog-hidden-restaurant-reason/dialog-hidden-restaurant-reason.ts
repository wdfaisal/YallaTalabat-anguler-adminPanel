...
@Component({
  selector: 'app-dialog-hidden-restaurant-reason',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-hidden-restaurant-reason.html',
})
export class DialogHiddenRestaurantReason {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogHiddenRestaurantReason>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/hide_reason/restaurant/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/hide_reason/restaurant/update/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
