...
import { AdminOutletListInterface } from 'src/app/interfaces/admin.outlet.list.interface';
...
export class Outlets {
  ...
  restaurants = new MatTableDataSource<AdminOutletListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/restaurant/getOutlets?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AdminOutletListInterface) => {
          ...
          this.restaurants = new MatTableDataSource<AdminOutletListInterface>(mappedList);
  ...
  onStatusChange(event: MatSlideToggleChange, restaurants: AdminOutletListInterface) {
    console.log(event);
  ...
  onEdit(restaurants: AdminOutletListInterface) {
    console.log(restaurants);
  ...
  onOutletManagerDetail(item: AdminOutletListInterface) {
    console.log(item);
    if (item && item.manager && item.manager.id && item.manager.id != '') {
      this.router.navigate(['admin/restaurant-management/restaurant-detail', item.manager.id]);
    }
  }

  onRestaurantDetail(item: AdminOutletListInterface) {
    console.log(item);
    this.router.navigate(['admin/restaurant-management/restaurant-detail', item.id]);
  }

  onCity(item: AdminOutletListInterface) {
    if (item && item.city && item.city.id != '') {
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/outlet_all_data/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
            const fileName = exportOption == 'excel' ? 'Outlets.xlsx' : 'Outlets.csv';
            this.api.download_export_file(blob, fileName);
          } else {
            const fileName = 'outlets.json';
            this.api.download_export_file(blob, fileName);
          }
  ...
  importCollection() {
    this.router.navigate(['admin/import-export-management/import-collection/', 'outlets']);
  }
