...
@Component({
  selector: 'app-dialog-restaurant-waiter',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-restaurant-waiter.html',
})
export class DialogRestaurantWaiter {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/admin/waiter/getById/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onSubmit() {
    ...
    this.api.patch_private('v1/admin/waiter/updateWaiterInfo/' + this.userId, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
