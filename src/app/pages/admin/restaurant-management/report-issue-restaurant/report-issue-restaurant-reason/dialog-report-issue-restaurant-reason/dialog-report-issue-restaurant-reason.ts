...
@Component({
  selector: 'app-dialog-report-issue-restaurant-reason',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-report-issue-restaurant-reason.html',
})
export class DialogReportIssueRestaurantReason {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogReportIssueRestaurantReason>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/report_issue/restaurant/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/report_issue/restaurant/update/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
