...
import { DialogRestaurantJoiningRejection } from './dialog-restaurant-joining-rejection/dialog-restaurant-joining-rejection';
...
@Component({
  selector: 'app-restaurant-joining-request-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, GoogleMapsModule, NgIcon],
  templateUrl: './restaurant-joining-request-detail.html',
})
export class RestaurantJoiningRequestDetail {
  ...
  getDetail() {
    ...
    this.api.get_private('v1/admin/restaurant_request/detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onRejectRequest() {
    ...
    const dialogRef = this.dialog.open(DialogRestaurantJoiningRejection, {
      data: { id: this.id },
      disableClose: true
    });
  ...
  onSubmit() {
    ...
    if (this.restaurantForm.valid) {
      ...
      this.api.post_private('v1/admin/restaurant_request/approve/' + this.id, sendData).subscribe({
        next: (response: any) => {
          console.log(response);
          this.util.stop(spinnerRef);
