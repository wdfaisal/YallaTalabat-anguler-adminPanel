...
@Component({
  selector: 'app-dialog-restaurant-facilities',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-restaurant-facilities.html',
})
export class DialogRestaurantFacilities {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogRestaurantFacilities>,
    ...
    if (this.data && this.data.action == 'edit') {
      ...
      this.id = values.id;
      this.facilitiesForm.controls['name'].setValue(values && values.name ? values.name : '');
  ...
  onSubmit() {
    ...
    const locale = this.facilitiesForm.get('translations') as FormArray;
    locale.clear();
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/restaurant/facilities/save', this.facilitiesForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        this.util.onSuccess('ts_facilities_added');
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/restaurant/facilities/update/' + this.id, this.facilitiesForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        this.util.onSuccess('ts_facilities_updated');
  ...
  get f() {
    return this.facilitiesForm.controls;
  }
