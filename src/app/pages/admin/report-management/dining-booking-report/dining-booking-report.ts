...
import { AdminDiningBookingReportInterface } from 'src/app/interfaces/admin.dining.booking.report.interface';
...
@Component({
  selector: 'app-dining-booking-report',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, MatNativeDateModule],
  templateUrl: './dining-booking-report.html',
})
export class DiningBookingReport {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  reports = new MatTableDataSource<AdminDiningBookingReportInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/reports/diningBooking?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AdminDiningBookingReportInterface) => {
          ...
          this.reports = new MatTableDataSource<AdminDiningBookingReportInterface>(mappedList);
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/reports/dining_booking/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
  ...
  onUserInfo(item: AdminDiningBookingReportInterface) {
  ...
  onRestaurantDetail(item: AdminDiningBookingReportInterface) {
    console.log(item);
