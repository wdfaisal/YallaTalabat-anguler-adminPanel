...
import { AdminPosOrderReportInterface } from 'src/app/interfaces/admin.pos.order.report.interface';
...
@Component({
  selector: 'app-pos-orders-report',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, MatNativeDateModule],
  templateUrl: './pos-orders-report.html',
})
export class PosOrdersReport {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  reports = new MatTableDataSource<AdminPosOrderReportInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/reports/posOrders?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AdminPosOrderReportInterface) => {
          ...
          this.reports = new MatTableDataSource<AdminPosOrderReportInterface>(mappedList);
  ...
  onOrderDetailPage(orderId: string) {
    console.log(orderId);
    this.router.navigate(['admin/order-management/vendors-pos-order-detail', orderId]);
  }
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/reports/pos_orders/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
            ...
            const fileName = 'posortableorders.json';
            this.api.download_export_file(blob, fileName);
          }
  ...
  onRestaurantDetail(item: AdminPosOrderReportInterface) {
    console.log(item);
