...
export class DialogRestaurantDisbursementDetail {
  ...
  getDetail() {
    ...
    this.api.get_private('v1/admin/disbursement/restaurantDisbursementDetail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onAccept() {
    console.log('Accept');
    this.isSubmit = true;
    this.api.get_private('v1/admin/disbursement/acceptRestaurantDisbursement/' + this.id).subscribe({
      next: (response: any) => {
        console.log(response);
