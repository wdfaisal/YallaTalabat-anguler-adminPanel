...
import { DeclineWithdrawalRequestDialog } from './decline-withdrawal-request-dialog/decline-withdrawal-request-dialog';
import { ApproveWithdrawalRequestDialog } from './approve-withdrawal-request-dialog/approve-withdrawal-request-dialog';
...
@Component({
  ...
  templateUrl: './withdrawal-request-detail.html',
})
export class WithdrawalRequestDetail {

  credential: AdminPayoutCredentialInterface[] = [];
  requestCredential: AdminPayoutCredentialInterface[] = [];
  id: string = '';
  amount: number = 0;
  createdAt: string = '';
  from: string = '';
  ...
  getDetail() {
    ...
    this.api.get_private('v1/admin/withdrawalRequest/detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onApproveRequest() {
    ...
    const dialogRef = this.dialog.open(ApproveWithdrawalRequestDialog, {
  ...
  onDeclineRequest() {
    ...
    const dialogRef = this.dialog.open(DeclineWithdrawalRequestDialog, {
      data: { id: this.id, balance: this.walletBalance, amount: this.amount },
      disableClose: true
    });
