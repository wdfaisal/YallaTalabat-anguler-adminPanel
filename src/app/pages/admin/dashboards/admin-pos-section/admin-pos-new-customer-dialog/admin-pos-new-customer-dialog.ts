...
@Component({
  selector: 'app-admin-pos-new-customer-dialog',
  imports: [MaterialModule, CommonModule, FormsModule, ReactiveFormsModule, NgIcon],
  templateUrl: './admin-pos-new-customer-dialog.html',
})
export class AdminPosNewCustomerDialog {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AdminPosNewCustomerDialog>,
  ) {
  ...
  onSubmit() {
    ...
    if (this.customerForm.valid) {
      ...
      this.isSubmit = true;
      this.api.post_private('v1/admin/pos/createCustomer', this.customerForm.getRawValue()).subscribe({
        next: (response: any) => {
