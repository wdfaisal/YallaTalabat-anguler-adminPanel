...
import { AdminPOSSectionCitiesInterface } from 'src/app/interfaces/admin.pos.section.cities.interface';
import { ApiService } from 'src/app/services/api-service';
import { UtilService } from 'src/app/services/util-service';
import { Observable, map, of, startWith } from 'rxjs';
import { FormControl } from '@angular/forms';
import { MatAutocompleteSelectedEvent } from '@angular/material/autocomplete';
import { AdminPOSSectionRestaurantInterface } from 'src/app/interfaces/admin.pos.section.restaurant.interface';
import { AdminPOSSectionCategoryInterface } from 'src/app/interfaces/admin.pos.section.category.interface';
...
import { AdminPosSelectCustomerDialog } from './admin-pos-select-customer-dialog/admin-pos-select-customer-dialog';
import { AdminPosNewCustomerDialog } from './admin-pos-new-customer-dialog/admin-pos-new-customer-dialog';
...
import { AdminPosCustomerAddressDialog } from './admin-pos-customer-address-dialog/admin-pos-customer-address-dialog';
import { AdminPOSSectionCustomerDeliveryAddressInterface } from 'src/app/interfaces/admin.pos.section.customer.delivery.address.interface';
import { DateTime } from 'luxon';
import { AdminPOSSectionDateInterface } from 'src/app/interfaces/admin.pos.section.date.interface';
import { MatSlideToggleChange } from '@angular/material/slide-toggle';
import { AdminPOSSectionCartItemRequestInterface, POSCartVariationConfig } from 'src/app/interfaces/admin.pos.section.cart.item.request.interface';
import { AdminPosFoodDetailDialog } from 'src/app/pages/admin/dashboards/admin-pos-section/admin-pos-food-detail-dialog/admin-pos-food-detail-dialog';
import { AdminPOSSectionTimeslotInterface } from 'src/app/interfaces/admin.pos.section.timeslot.interface';
import { AdminPosSectionCustomerDetailSchemaInterface } from 'src/app/interfaces/admin.pos.section.customer.detail.schema.interface';
...
@Component({
  selector: 'app-admin-pos-section',
  imports: [MaterialModule, CommonModule, FormsModule, ReactiveFormsModule, NgIcon],
  templateUrl: './admin-pos-section.html',
})
export class AdminPosSection {
  cities: Observable<AdminPOSSectionCitiesInterface[]>;
  listOfCities: AdminPOSSectionCitiesInterface[] = [];
  cityCtrl = new FormControl('');
  cityId: string = '';
  cityLat: number = 0;
  cityLng: number = 0;
  restaurants: Observable<AdminPOSSectionRestaurantInterface[]>;
  listOfRestaurants: AdminPOSSectionRestaurantInterface[] = [];
  ...
  categories: AdminPOSSectionCategoryInterface[] = [];
  foodKind: string = '';
  categoryId: string = '';
  ...
  customerDetail = new MatTableDataSource<AdminPosSectionCustomerDetailSchemaInterface>([]);
  customerTableColumns: string[] = ['name', 'info'];
  customerDeliveryAddress: AdminPOSSectionCustomerDeliveryAddressInterface = {
    receiverName: '',
    flatHouse: '',
    locality: '',
    countryCode: '',
  ...
  dateList: AdminPOSSectionDateInterface[] = [];
  dateIndex: number = 0;
  ...
  getCities() {
    ...
    this.api.get_private('v1/admin/pos/initial/').subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        if (response && response.success == true && response.cities) {
          this.listOfCities = response.cities;
          this.cities = this.cityCtrl.valueChanges.pipe(
            startWith(''),
            map((element) => (element ? this._filterCity(element) : this.listOfCities.slice()))
          );
        }
  ...
  private _filterCity(value: any): AdminPOSSectionCitiesInterface[] {
  ...
  onCitySelect(item: MatAutocompleteSelectedEvent) {
    console.log(item);
    if (item && item.option && item.option.value) {
      const city = this.listOfCities.find((x) => x.id == item.option.value);
      if (city && city.id) {
        ...
        this.calculateItemTotal();
        this.cityId = city.id;
        if (city.location && city.location.coordinates && city.location.coordinates.length > 0) {
          const coordinates = city.location.coordinates;
          this.cityLat = coordinates[1];
          this.cityLng = coordinates[0];
        }
        this.fetchRestaurant(city.id);
      }
    }

  }

  fetchRestaurant(city: string) {
    console.log(city);
    const spinnerRef = this.util.start('ts_fetching');
    this.api.get_private('v1/admin/pos/restaurants/' + city).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        if (response && response.success == true && response.restaurants) {
  ...
  onRestaurantSelect(item: MatAutocompleteSelectedEvent) {
    console.log(item);
    if (item && item.option && item.option.value) {
      this.restaurantId = item.option.value;
      this.categoryIndex = 0;
      this.categories = [];
      this.foodKind = '';
      this.categoryId = '';
  ...
  getRestaurantData() {
    ...
    this.api.get_private('v1/admin/pos/categories/' + this.restaurantId).subscribe({
      next: (response: any) => {
        console.log(response);
        this.foodFetch = true;
        this.categories = [];
  ...
  private _filterRestaurant(value: any): AdminPOSSectionRestaurantInterface[] {
    let filterValue: string;
    if (value && value.name) {
  ...
  searchProducts(event: Event): void {
    ...
    if (filterValue.length >= 3) {
      this.foodFetch = false;
      this.products = [];
      this.api.get_private('v1/admin/pos/search/' + this.restaurantId + '/' + filterValue).subscribe({
        next: (response: any) => {
          console.log(response);
          this.foodFetch = true;
  ...
  getFoodList() {
    ...
    console.log(param);
    this.products = [];
    this.api.post_private('v1/admin/pos/food_list/', param).subscribe({
      next: (response: any) => {
        console.log(response);
        this.foodFetch = true;
        if (response && response.success && response.products && response.products.length) {
          response.products.forEach((element: any) => {
            ...
            console.log(`Name --> ${element.name} Offer Price --> ${offerPrice}`);
  ...
  onFoodInfo(item: PosFoodInfoInterface) {
    console.log(item);
    const dialogRef = this.dialog.open(AdminPosFoodDetailDialog, {
      disableClose: true,
      autoFocus: false,
      restoreFocus: false,
      data: { info: item },
  ...
  onUserModal() {
    ...
    const dialogRef = this.dialog.open(AdminPosSelectCustomerDialog, {
      disableClose: true,
      autoFocus: false,
      restoreFocus: false,
  ...
  onNewCustomer() {
    const dialogRef = this.dialog.open(AdminPosNewCustomerDialog, {
      disableClose: true
    });
  ...
  getCustomerDetail() {
    ...
    this.api.get_private('v1/admin/pos/customer_detail/' + this.customerId).subscribe({
      next: (response: any) => {
        console.log(response);
        ...
        if (response && response.success) {
          ...
          const userDetailTable: AdminPosSectionCustomerDetailSchemaInterface[] = [];
  ...
  onSubmit() {
    console.log('Submit');
    const cartItems: AdminPOSSectionCartItemRequestInterface[] = [];
    ...
    this.api.post_private('v1/admin/pos/place_order/', param).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  addDeliveryAddress() {
    const dialogRef = this.dialog.open(AdminPosCustomerAddressDialog, {
      disableClose: true,
      autoFocus: false,
      restoreFocus: false,
      data: {
  ...
  getTranslatedCityName(item: AdminPOSSectionCitiesInterface): string {
  ...
  getTranslatedRestaurantName(item: AdminPOSSectionRestaurantInterface): string {
    if (item.translations && Array.isArray(item.translations)) {
      const found = item.translations.find(t => t.code == this.util.appLocaleName());
