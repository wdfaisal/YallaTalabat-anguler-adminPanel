...
import { AdminDeliverymanJoiningRequestInterface } from 'src/app/interfaces/admin.deliveryman.joining.request.interface';
...
@Component({
  selector: 'app-deliveryman-joining-request',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './deliveryman-joining-request.html',
})
export class DeliverymanJoiningRequest {

  @ViewChild('paginator', { read: MatPaginator }) paginator: MatPaginator;
  deliveryman = new MatTableDataSource<AdminDeliverymanJoiningRequestInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/deliveryman_request/list/' + this.requestType + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AdminDeliverymanJoiningRequestInterface) => {
          ...
          this.deliveryman = new MatTableDataSource<AdminDeliverymanJoiningRequestInterface>(mappedList);
  ...
  onInfo(request: AdminDeliverymanJoiningRequestInterface) {
    console.log(request);
    this.router.navigate(['admin/driver-management/deliveryman-joining-request-detail', request.id]);
  }

  onDeleteRequest(request: AdminDeliverymanJoiningRequestInterface) {
    console.log(request);
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result && result.action && result.action == "confirm") {
        ...
        this.api.delete_private('v1/admin/deliveryman_request/delete/' + request.id).subscribe({
          next: (response: any) => {
            console.log(response);
  ...
  onCity(item: AdminDeliverymanJoiningRequestInterface) {
    if (item && item.city && item.city.id != '') {
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/deliveryman_request/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
            const fileName = exportOption == 'excel' ? 'DeliverymanJoiningRequest.xlsx' : 'DeliverymanJoiningRequest.csv';
            this.api.download_export_file(blob, fileName);
          } else {
            const fileName = 'deliverymanjoiningrequests.json';
            this.api.download_export_file(blob, fileName);
          }
