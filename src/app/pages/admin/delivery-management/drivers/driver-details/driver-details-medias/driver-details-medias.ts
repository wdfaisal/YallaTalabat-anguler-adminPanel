...
@Component({
  selector: 'app-driver-details-medias',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgScrollbarModule],
  templateUrl: './driver-details-medias.html',
})
export class DriverDetailsMedias implements AfterViewInit {
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/deliveryman_detail/media_files?' + httpParams.toString()).subscribe({
      next: (response: any) => {
