...
@Component({
  selector: 'app-dialog-media-info',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './dialog-media-info.html',
})
export class DialogMediaInfo {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogMediaInfo>,
  ...
  onDelete() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result && result.action && result.action == "confirm") {
        ...
        this.api.delete_private('v1/admin/files/delete/' + this.id).subscribe({
          next: (response: any) => {
