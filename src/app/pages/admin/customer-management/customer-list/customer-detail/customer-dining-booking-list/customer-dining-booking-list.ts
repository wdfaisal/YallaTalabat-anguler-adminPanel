...
import { AdminCustomerDiningBookingInterface } from 'src/app/interfaces/admin.customer.dining.booking.interface';
...
@Component({
  selector: 'app-customer-dining-booking-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './customer-dining-booking-list.html',
})
export class CustomerDiningBookingList implements AfterViewInit {

  @Input() userId!: string;
  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  bookings = new MatTableDataSource<AdminCustomerDiningBookingInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/customer/diningBookingList?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.bookings) {
          const mappedList = response.bookings.map(
            (item: AdminCustomerDiningBookingInterface) => {
          ...
          this.bookings = new MatTableDataSource<AdminCustomerDiningBookingInterface>(mappedList);
