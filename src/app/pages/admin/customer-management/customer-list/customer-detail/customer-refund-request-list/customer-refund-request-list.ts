...
import { AdminCustomerBookingRefundRequestInterface } from 'src/app/interfaces/admin.customer.booking.refund.request.interface';
import { AdminCustomerOrderRefundRequestInterface } from 'src/app/interfaces/admin.customer.order.refund.request.interface';
import { AdminCustomerTiffinPackageRefundRequestInterface } from 'src/app/interfaces/admin.customer.tiffin.package.refund.request.interface';
...
@Component({
  selector: 'app-customer-refund-request-list',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './customer-refund-request-list.html',
})
export class CustomerRefundRequestList implements AfterViewInit {
  ...
  orderRefundList = new MatTableDataSource<AdminCustomerOrderRefundRequestInterface>([]);
  tiffinRefundList = new MatTableDataSource<AdminCustomerTiffinPackageRefundRequestInterface>([]);
  bookingRefundList = new MatTableDataSource<AdminCustomerBookingRefundRequestInterface>([]);
  ...
  getInitial() {
    ...
    this.api.get_private('v1/admin/customer/customerAllRefundRequest?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.orderRefund && response.orderRefund.orderRefundRequest) {
          const mappedList = response.orderRefund.orderRefundRequest.map(
            (item: AdminCustomerOrderRefundRequestInterface) => {
          ...
          this.orderRefundList = new MatTableDataSource<AdminCustomerOrderRefundRequestInterface>(mappedList);
        ...
        if (response && response.tiffinRefund && response.tiffinRefund.tiffinRefundRequest) {
          const mappedList = response.tiffinRefund.tiffinRefundRequest.map(
            (item: AdminCustomerTiffinPackageRefundRequestInterface) => {
              if (item && item.restaurant && item.restaurant?.id) {
                if (item.restaurant?.translations) {
                  const translation = item.restaurant.translations.find((t) => t.code == this.util.appLocaleName());
                  item.restaurant.displayName = translation?.title || item.restaurant.name;
          ...
          this.tiffinRefundList = new MatTableDataSource<AdminCustomerTiffinPackageRefundRequestInterface>(mappedList);
        ...
        if (response && response.bookingRefund && response.bookingRefund.bookingRefundRequest) {
          const mappedList = response.bookingRefund.bookingRefundRequest.map(
            (item: AdminCustomerBookingRefundRequestInterface) => {
              if (item && item.restaurant && item.restaurant?.id) {
                if (item.restaurant?.translations) {
                  const translation = item.restaurant.translations.find((t) => t.code == this.util.appLocaleName());
                  item.restaurant.displayName = translation?.title || item.restaurant.name;
          ...
          this.bookingRefundList = new MatTableDataSource<AdminCustomerBookingRefundRequestInterface>(mappedList);
  ...
  getOrderRefundList() {
    ...
    this.api.get_private('v1/admin/customer/customerOrderRefundList?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedOrders = true;
  ...
  getTiffinRefundList() {
    ...
    this.api.get_private('v1/admin/customer/customerTiffinRefundList?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedTiffin = true;
        if (response && response.results) {
  ...
  getBookingRefundList() {
    ...
    this.api.get_private('v1/admin/customer/customerBookingRefundList?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoadedBooking = true;
        if (response && response.results) {
