...
import { AdminRestaurantDeletedAccountListInterface } from 'src/app/interfaces/admin.restaurant.deleted.account.list.interface';
...
@Component({
  selector: 'app-restaurant-deleted-accounts',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './restaurant-deleted-accounts.html',
})
export class RestaurantDeletedAccounts {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  restaurants = new MatTableDataSource<AdminRestaurantDeletedAccountListInterface>([]);
  displayedColumn = ['name', 'reason', 'city', 'foods', 'orders', 'pos', 'table', 'dining', 'permission', 'commission', 'rating', 'datetime'];
  pageSize: number = 5;
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/restaurants/deleted_restaurants?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AdminRestaurantDeletedAccountListInterface) => {
              ...
              if (item && item.subscriptionInfo && item.subscriptionInfo?.name) {
          ...
          this.restaurants = new MatTableDataSource<AdminRestaurantDeletedAccountListInterface>(mappedList);
  ...
  onCity(item: AdminRestaurantDeletedAccountListInterface) {
    if (item && item.city && item.city.id != '') {
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/admin/restaurants/deleted_restaurants/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          if (exportOption != 'raw') {
            const fileName = exportOption == 'excel' ? 'RestaurantDeletedAccounts.xlsx' : 'RestaurantDeletedAccounts.csv';
            this.api.download_export_file(blob, fileName);
          } else {
            const fileName = 'deletedrestaurantaccounts.json';
            this.api.download_export_file(blob, fileName);
          }
