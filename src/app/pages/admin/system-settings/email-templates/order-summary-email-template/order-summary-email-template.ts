...
@Component({
  selector: 'app-order-summary-email-template',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './order-summary-email-template.html',
})
export class OrderSummaryEmailTemplate extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'order-summary';
    console.log('Dynamic Content For Order -->##ORDERSUMMARY##');
    this.dynamicContentName = '##ORDERSUMMARY##';
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.orderSummary;
          }
        }
      }, error: (error: any) => {
        console.log(error);
