...
@Component({
  selector: 'app-deliveryman-reject-request-email-template',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './deliveryman-reject-request-email-template.html',
})
export class DeliverymanRejectRequestEmailTemplate extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'deliveryman-register-request-rejected';
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.rejected;
          }
        }
      }, error: (error: any) => {
        console.log(error);
