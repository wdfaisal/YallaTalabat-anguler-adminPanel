...
@Component({
  selector: 'app-support-chat-export-email-template',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './support-chat-export-email-template.html',
})
export class SupportChatExportEmailTemplate extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'support-chat-export';
    console.log('Dynamic Content For OTP -->##OTP##');
    this.dynamicContentName = '##CHATS##';
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.supportChatExport;
          }
        }
      }, error: (error: any) => {
        console.log(error);
