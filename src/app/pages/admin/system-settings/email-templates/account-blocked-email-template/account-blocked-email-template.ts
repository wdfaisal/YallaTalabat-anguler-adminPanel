...
@Component({
  selector: 'app-account-blocked-email-template',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './account-blocked-email-template.html',
})
export class AccountBlockedEmailTemplate extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'account-blocked-email';
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.accountBlocked;
          }
        }
      }, error: (error: any) => {
        console.log(error);
