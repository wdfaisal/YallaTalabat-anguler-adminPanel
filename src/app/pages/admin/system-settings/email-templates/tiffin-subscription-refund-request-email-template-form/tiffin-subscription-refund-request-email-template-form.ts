...
@Component({
  selector: 'app-tiffin-subscription-refund-request-email-template-form',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './tiffin-subscription-refund-request-email-template-form.html',
})
export class TiffinSubscriptionRefundRequestEmailTemplateForm extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'user-tiffin-subscription-refund-request';
    console.log('Dynamic Content For OTP -->##OTP##');
    this.dynamicContentName = '##REASON##';
    console.log(`TO SAVE DYNAMIC CONTENT NAME ___>>>> ${this.dynamicContentName}`);
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.refundRequest;
          }
        }
      }, error: (error: any) => {
        console.log(error);
