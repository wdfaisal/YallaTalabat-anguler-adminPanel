...
@Component({
  selector: 'app-deliveryman-approve-request-email-template',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './deliveryman-approve-request-email-template.html',
})
export class DeliverymanApproveRequestEmailTemplate extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'deliveryman-register-request-approved';
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.approved;
          }
        }
      }, error: (error: any) => {
        console.log(error);
