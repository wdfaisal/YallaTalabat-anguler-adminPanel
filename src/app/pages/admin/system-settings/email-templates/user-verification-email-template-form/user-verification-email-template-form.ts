...
@Component({
  selector: 'app-user-verification-email-template-form',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './user-verification-email-template-form.html',
})
export class UserVerificationEmailTemplateForm extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'user-verification';
    console.log('Dynamic Content For OTP -->##OTP##');
    this.dynamicContentName = '##OTP##';
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.userVerification;
          }
        }
      }, error: (error: any) => {
        console.log(error);
