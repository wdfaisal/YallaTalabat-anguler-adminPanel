...
@Component({
  selector: 'app-report-emergency-thank-you-form',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './report-emergency-thank-you-form.html',
})
export class ReportEmergencyThankYouForm extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'report-emergency-thank-you';
    console.log('Dynamic Content For REPLY -->##REPLY##');
    this.dynamicContentName = '##REPLY##';
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.gratitude;
          }
        }
      }, error: (error: any) => {
        console.log(error);
