...
@Component({
  selector: 'app-restaurant-expired-package-blocked-email-template',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, AngularEditorModule, SafeHtmlPipe],
  templateUrl: './restaurant-expired-package-blocked-email-template.html',
})
export class RestaurantExpiredPackageBlockedEmailTemplate extends EmailTemplatePreviewBase {
  ...
  constructor(
    ...
    this.slug = 'restaurant-package-expired-account-blocked-email';
      ...
      next: (dataResponse: any) => {
        ...
        if (dataResponse && dataResponse.media) {
          if (dataResponse.media && dataResponse.media.mediaUrls) {
            ...
            this.topImageLogo = mediaUrls.restaurantAccountBlocked;
          }
        }
      }, error: (error: any) => {
        console.log(error);
