...
@Component({
  ...
  templateUrl: './paystack-payment.html',
})
export class PaystackPayment {
  ...
  constructor(
    ...
    this.paymentForm.controls['slug'].patchValue('paystack');
    this.locale();
    this.getPaymentInfo();
  }
      ...
      next: (response: any) => {
        ...
        if (response && response.id) {
          ...
          if (response && response.credentials && response.credentials instanceof Object) {
            this.paymentForm.controls['credentials'].controls['sk'].setValue(response && response.credentials && response.credentials.sk ? response.credentials.sk : '');
            this.paymentForm.controls['credentials'].controls['pk'].setValue(response && response.credentials && response.credentials.pk ? response.credentials.pk : '');
          }
