...
@Component({
  ...
  templateUrl: './paytm-payment.html',
})
export class PaytmPayment {
  ...
  constructor(
    ...
    this.paymentForm.controls['slug'].patchValue('paytm');
    this.locale();
    this.getPaymentInfo();
  }
      ...
      next: (response: any) => {
        ...
        if (response && response.id) {
          ...
          if (response && response.credentials && response.credentials instanceof Object) {
            this.paymentForm.controls['credentials'].controls['key'].setValue(response && response.credentials && response.credentials.key ? response.credentials.key : '');
            this.paymentForm.controls['credentials'].controls['mid'].setValue(response && response.credentials && response.credentials.mid ? response.credentials.mid : '');
            this.paymentForm.controls['credentials'].controls['stage'].setValue(response && response.credentials && response.credentials.stage ? response.credentials.stage : '');
          }
