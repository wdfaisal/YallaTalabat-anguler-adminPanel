...
@Component({
  selector: 'app-cashfree-payment',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './cashfree-payment.html',
})
export class CashfreePayment {
  ...
  constructor(
    ...
    this.paymentForm.controls['slug'].patchValue('cashfree');
    this.locale();
    this.getPaymentInfo();
  }
      ...
      next: (response: any) => {
        ...
        if (response && response.id) {
          ...
          if (response && response.credentials && response.credentials instanceof Object) {
            this.paymentForm.controls['credentials'].controls['appId'].setValue(response && response.credentials && response.credentials.appId ? response.credentials.appId : '');
            this.paymentForm.controls['credentials'].controls['secretKey'].setValue(response && response.credentials && response.credentials.secretKey ? response.credentials.secretKey : '');
            this.paymentForm.controls['credentials'].controls['apiVersion'].setValue(response && response.credentials && response.credentials.apiVersion ? response.credentials.apiVersion : '');
          }
  ...
  onReset() {
    ...
    (this.paymentForm.get('translations') as FormArray).clear();
    this.paymentForm.controls['slug'].patchValue('cashfree');
