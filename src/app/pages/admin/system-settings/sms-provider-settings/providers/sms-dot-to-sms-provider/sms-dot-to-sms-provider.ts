...
@Component({
  selector: 'app-sms-dot-to-sms-provider',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './sms-dot-to-sms-provider.html',
})
export class SmsDotToSmsProvider {
  ...
  constructor(
    ...
    this.smsProviderForm.controls['slug'].patchValue('sms_dot_to');
    this.locale();
    this.getDetail();
  }
      ...
      next: (response: any) => {
        ...
        if (response && response.id) {
          ...
          if (response && response.credentials && response.credentials instanceof Object) {
            ...
            console.log(creds);
            this.smsProviderForm.controls['credentials'].controls['clientId'].setValue(creds && creds.clientId ? creds.clientId : '');
            this.smsProviderForm.controls['credentials'].controls['clientSecret'].setValue(creds && creds.clientSecret ? creds.clientSecret : '');
  ...
  onDemoSubmit() {
    ...
    if (this.demoForm.valid) {
      ...
      this.api.post_private('v1/admin/sms_provider/demo_sms_dot_to/', param).subscribe({
        next: (response: any) => {
          console.log(response);
          this.util.stop(spinnerRef);
