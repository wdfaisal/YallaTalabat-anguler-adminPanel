...
@Component({
  selector: 'app-manage-restaurant-campaign',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './manage-restaurant-campaign.html',
})
export class ManageRestaurantCampaign {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/admin/restaurant_campaign/getById/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onSaveCampaign() {
    ...
    this.api.post_private('v1/admin/restaurant_campaign/save', sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        this.util.onSuccess('ts_restaurant_campaign_added');
        this.router.navigate(['admin/promotion-management/restaurant-campaign']);
      }, error: (error: any) => {
        this.util.stop(spinnerRef);
        this.util.handleError(error, 'admin');
      }
  ...
  onUpdateCampaign() {
    ...
    console.log(sendData);
    const spinnerRef = this.util.start('ts_saving');
    this.api.patch_private('v1/admin/restaurant_campaign/update/' + this.id, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
