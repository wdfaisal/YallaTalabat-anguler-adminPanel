...
import { AdminVendorCoupoRequestListInterface } from 'src/app/interfaces/admin.vendor.coupon.request.list.interface';
...
@Component({
  ...
  templateUrl: './coupon-request.html',
})
export class CouponRequest {

  @ViewChild('paginator', { read: MatPaginator }) paginator: MatPaginator;
  coupons = new MatTableDataSource<AdminVendorCoupoRequestListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/coupon/request?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.coupons) {
          const mappedList = response.coupons.map(
            (item: AdminVendorCoupoRequestListInterface) => {
          ...
          this.coupons = new MatTableDataSource<AdminVendorCoupoRequestListInterface>(mappedList);
  ...
  onEdit(coupon: AdminVendorCoupoRequestListInterface) {
    console.log(coupon);
  ...
  onAccept(coupon: AdminVendorCoupoRequestListInterface) {
    console.log('Accept');
    const spinnerRef = this.util.start();
    this.api.patch_private('v1/admin/coupon/updateMeta/' + coupon.id, { status: 'live' }).subscribe({
      next: (response: any) => {
        console.log(response);
  ...
  onReject(coupon: AdminVendorCoupoRequestListInterface) {
  ...
  onRestaurantDetail(item: AdminVendorCoupoRequestListInterface) {
    console.log(item);
  ...
  onCity(item: AdminVendorCoupoRequestListInterface) {
    if (item && item.city && item.city.id != '') {
