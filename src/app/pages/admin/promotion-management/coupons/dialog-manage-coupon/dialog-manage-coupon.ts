...
@Component({
  selector: 'app-dialog-manage-coupon',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-manage-coupon.html',
})
export class DialogManageCoupon {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/admin/coupon/getInfo/' + this.id).subscribe({
      next: (response: any) => {
  ...
  saveCoupon() {
    ...
    this.api.post_private('v1/admin/coupon/save', sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  updateCoupon() {
    ...
    this.api.patch_private('v1/admin/coupon/update/' + this.id, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  async onUserModal() {
    if (this.couponForm.controls['allUsers'].value == false) {
      ...
      const dialogRef = await this.dialog.open(DialogUsers, {
