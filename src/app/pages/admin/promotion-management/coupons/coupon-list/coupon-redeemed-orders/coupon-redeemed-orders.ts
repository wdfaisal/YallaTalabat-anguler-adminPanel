...
@Component({
  selector: 'app-coupon-redeemed-orders',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './coupon-redeemed-orders.html',
})
export class CouponRedeemedOrders {
  ...
  getDetail() {
    ...
    this.api.get_private('v1/admin/coupon/detail/' + this.id + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        this.isLoaded = true;
  ...
  getList() {
    ...
    this.api.get_private('v1/admin/orders/coupon/' + this.id + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
