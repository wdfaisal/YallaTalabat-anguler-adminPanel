...
@Component({
  selector: 'app-dialog-accountant-team-role',
  imports: [FormsModule, ReactiveFormsModule, MaterialModule, CommonModule, NgIcon],
  templateUrl: './dialog-accountant-team-role.html',
})
export class DialogAccountantTeamRole {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogAccountantTeamRole>,
  ...
  getInfo() {
    ...
    this.api.get_private('v1/admin/auth_roles/detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onCreate() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/auth_roles/new_accountant', this.roleForm.getRawValue()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        if (response && response.user && response.user.id) {
          this.util.onSuccess('ts_accountant_saved');
  ...
  onUpdate() {
    console.log('Update--');
    this.isSubmit = true;
    this.api.patch_private('v1/admin/auth_roles/update_detail/' + this.id, this.roleForm.getRawValue()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        if (response && response.success) {
          this.util.onSuccess('ts_accountant_updated');
