...
@Component({
  selector: 'app-dialog-admin-role',
  imports: [FormsModule, ReactiveFormsModule, MaterialModule, CommonModule, NgIcon],
  templateUrl: './dialog-admin-role.html',
})
export class DialogAdminRole {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogAdminRole>,
  ...
  onCreate() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/auth_roles/new_admin', this.roleForm.getRawValue()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        if (response && response.user && response.user.id) {
          this.util.onSuccess('ts_admin_saved');
    ...
    this.api.patch_private('v1/admin/auth_roles/update_detail/' + this.id, this.roleForm.getRawValue()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.success) {
          this.util.onSuccess('ts_admin_updated');
