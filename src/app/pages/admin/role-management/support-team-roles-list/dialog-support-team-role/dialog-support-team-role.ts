...
@Component({
  selector: 'app-dialog-support-team-role',
  imports: [FormsModule, ReactiveFormsModule, MaterialModule, CommonModule, NgIcon],
  templateUrl: './dialog-support-team-role.html',
})
export class DialogSupportTeamRole {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogSupportTeamRole>,
  ...
  onCreate() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/auth_roles/new_support_team', this.roleForm.getRawValue()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        if (response && response.user && response.user.id) {
          this.util.onSuccess('ts_support_team_saved');
    ...
    this.api.patch_private('v1/admin/auth_roles/update_detail/' + this.id, this.roleForm.getRawValue()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.success) {
          this.util.onSuccess('ts_support_team_updated');
