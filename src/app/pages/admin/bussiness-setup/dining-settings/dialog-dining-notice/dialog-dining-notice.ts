...
@Component({
  selector: 'app-dialog-dining-notice',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-dining-notice.html',
})
export class DialogDiningNotice {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogDiningNotice>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/dining/notice/save', this.noticeForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/dining/notice/update/' + this.id, this.noticeForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
