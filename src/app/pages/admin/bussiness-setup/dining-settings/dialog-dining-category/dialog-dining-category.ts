...
@Component({
  selector: 'app-dialog-dining-category',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-dining-category.html',
})
export class DialogDiningCategory {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogDiningCategory>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/dining_category/save', this.categoryForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/dining_category/update/' + this.id, this.categoryForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
