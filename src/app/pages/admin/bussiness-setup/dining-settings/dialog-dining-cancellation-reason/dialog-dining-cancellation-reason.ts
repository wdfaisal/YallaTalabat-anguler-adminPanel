...
@Component({
  selector: 'app-dialog-dining-cancellation-reason',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-dining-cancellation-reason.html',
})
export class DialogDiningCancellationReason {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogDiningCancellationReason>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/dining_cancel_reason/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/dining_cancel_reason/update/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
