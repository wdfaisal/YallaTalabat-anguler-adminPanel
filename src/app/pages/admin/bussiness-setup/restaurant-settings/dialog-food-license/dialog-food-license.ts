...
@Component({
  selector: 'app-dialog-food-license',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-food-license.html',
})
export class DialogFoodLicense {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogFoodLicense>,
    ...
    if (this.data && this.data.action == 'edit') {
      ...
      this.licenseForm.controls['website'].setValue(values && values.website ? values.website : '');
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/restaurant_food_license/save', this.licenseForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        this.util.onSuccess('ts_license_added');
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/restaurant_food_license/update/' + this.id, this.licenseForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        this.util.onSuccess('ts_license_updated');
