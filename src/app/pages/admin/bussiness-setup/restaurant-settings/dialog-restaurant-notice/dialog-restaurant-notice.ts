...
@Component({
  selector: 'app-dialog-restaurant-notice',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-restaurant-notice.html',
})
export class DialogRestaurantNotice {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogRestaurantNotice>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/restaurant/notice/save', this.noticeForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/restaurant/notice/update/' + this.id, this.noticeForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
