...
@Component({
  selector: 'app-dialog-invoice-instruction',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-invoice-instruction.html',
})
export class DialogInvoiceInstruction {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogInvoiceInstruction>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/invoice/instruction/save', this.noticeForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/invoice/instruction/update/' + this.id, this.noticeForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
