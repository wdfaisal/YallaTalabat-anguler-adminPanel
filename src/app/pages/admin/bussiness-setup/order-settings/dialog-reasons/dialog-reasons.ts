...
@Component({
  selector: 'app-dialog-reasons',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-reasons.html',
})
export class DialogReasons {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogReasons>,
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/order_cancel_reason/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/order_cancel_reason/update/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
