...
@Component({
  selector: 'app-dialog-user-account-delete-reason',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-user-account-delete-reason.html',
})
export class DialogUserAccountDeleteReason {

  action: string = 'create';
  reasonForm = new FormGroup({
    name: new FormControl('', [Validators.required]),
    kind: new FormControl('user', [Validators.required]),
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogUserAccountDeleteReason>,
    ...
    if (this.data && this.data.action == 'edit') {
      ...
      this.reasonForm.controls['kind'].setValue(values && values.kind ? values.kind : '');
  ...
  onSave() {
    this.isSubmit = true;
    this.api.post_private('v1/admin/delete_account_reason/save', this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        this.util.onSuccess('ts_notice_added');
  ...
  onUpdate() {
    this.isSubmit = true;
    this.api.patch_private('v1/admin/update_delete_account_reason/' + this.id, this.reasonForm.value).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isSubmit = false
        this.util.onSuccess('ts_notice_updated');
