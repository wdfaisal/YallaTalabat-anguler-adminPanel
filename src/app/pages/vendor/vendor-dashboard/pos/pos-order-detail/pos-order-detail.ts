...
@Component({
  ...
  templateUrl: './pos-order-detail.html',
})
export class PosOrderDetail {
  ...
  getOrderDetail() {
    ...
    this.api.get_private('v1/vendor_web/restaurant/posOrderDetail/' + this.id + '/' + this.util.getItem('_vendorId')).subscribe({
      next: (response: any) => {
  ...
  onPrintInvoice() {
    this.router.navigate(['vendor/pos-management/pos-order-print-invoice/', this.id]);
  }
