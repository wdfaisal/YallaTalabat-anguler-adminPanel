...
import { VendorDiningCampaignListInterface } from 'src/app/interfaces/vendor.dining.campaign.list.interface';
...
export class DiningCampaign {
  ...
  campaigns = new MatTableDataSource<VendorDiningCampaignListInterface>([]);
  ...
  getList() {
    ...
    this.api.get_private('v1/vendor_web/dining_campaign/near_me/' + this.util.getItem('_vendorId') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        this.isLoaded = true;
        console.log(response);
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: VendorDiningCampaignListInterface) => {
          ...
          this.campaigns = new MatTableDataSource<VendorDiningCampaignListInterface>(mappedList);
  ...
  joined(element: VendorDiningCampaignListInterface) {
  ...
  onAction(element: VendorDiningCampaignListInterface) {
    if (element && element.restaurant && element.restaurant.length && element.restaurant.length > 0 && element.restaurant.includes(this.util.getItem('_vendorId'))) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result && result.action && result.action == "confirm") {
          ...
          this.api.delete_private('v1/vendor_web/dining_campaign/leave/' + element.id + '/' + this.util.getItem('_vendorId')).subscribe({
            next: (response: any) => {
              console.log(response);
      ...
      this.api.post_private('v1/vendor_web/dining_campaign/join_request', param).subscribe({
        next: (response: any) => {
          console.log(response);
          this.util.stop(spinnerRef);
