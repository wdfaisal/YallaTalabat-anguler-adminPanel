...
@Component({
  selector: 'app-completed-table-order-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './completed-table-order-detail.html',
})
export class CompletedTableOrderDetail {
  ...
  getOrderDetail() {
    ...
    this.api.get_private('v1/vendor_web/table_order/tableOrderDetail/' + this.util.getItem('_vendorId') + '/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onPrintInvoice() {
    this.router.navigate(['vendor/table-management/completed-table-order-invoice-print/', this.id]);
  }
