...
@Component({
  selector: 'app-table-order-invoice-print',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './table-order-invoice-print.html',
})
export class TableOrderInvoicePrint {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/vendor_web/table_order/invoice_print/' + this.id + '/' + this.util.getItem('_vendorId')).subscribe({
      next: (response: any) => {
        ...
        if (response && response.success == true) {
          ...
          if (details && details.tableDetail && details.tableDetail.id && details.tableDetail.id != '') {
            this.tableNumber = details.tableDetail.tableNumber;
          }
    ...
    const hardcodedHTML = `
      <!DOCTYPE html>
        ...
        <body>
          ...
          <p class="lbl" style="width:${printPaddingWidth}">${this.util.appTranslate('table_no')}: #${this.tableNumber}</p>
