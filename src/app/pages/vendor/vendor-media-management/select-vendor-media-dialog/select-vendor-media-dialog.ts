...
import { VendorMediaListDialogInterface } from 'src/app/interfaces/vendor.media.list.dialog.interface';
...
@Component({
  selector: 'app-select-vendor-media-dialog',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './select-vendor-media-dialog.html',
})
export class SelectVendorMediaDialog {

  listOfMedias: VendorMediaListDialogInterface[] = [];
  pageSize: number = 12;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<SelectVendorMediaDialog>,
  ...
  getList() {
    ...
    this.api.get_private('v1/vendor_web/media/getVendorMedia/' + this.util.getItem('_uid') + '?' + httpParams.toString()).subscribe({
      next: (response: any) => {
