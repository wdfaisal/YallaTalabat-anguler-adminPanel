...
@Component({
  selector: 'app-vendor-profile-setting',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './vendor-profile-setting.html',
})
export class VendorProfileSetting {
  ...
  getDetail() {
    ...
    this.api.get_private('v1/vendor_web/vendor_profile/' + this.util.getItem('_uid')).subscribe({
      next: (response: any) => {
    ...
    if (this.form.valid) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result && result.action && result.action == "confirm") {
          ...
          this.api.patch_private('v1/vendor_web/update_vendor/' + this.util.getItem('_uid'), this.form.getRawValue()).subscribe({
            next: (response: any) => {
              console.log(response);
              this.util.stop(spinnerRef);
    ...
    if (this.passwordForm.valid) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result && result.action && result.action == "confirm") {
          console.log('confirmed');
          const spinnerRef = this.util.start('ts_updating');
          this.api.patch_private('v1/vendor_web/update_vendor_password/' + this.util.getItem('_uid'), { "password": this.passwordForm.controls['password'].value }).subscribe({
            next: (response: any) => {
              console.log(response);
              this.util.stop(spinnerRef);
              this.api.post_public('v1/auth/logout_web', {}).subscribe({
