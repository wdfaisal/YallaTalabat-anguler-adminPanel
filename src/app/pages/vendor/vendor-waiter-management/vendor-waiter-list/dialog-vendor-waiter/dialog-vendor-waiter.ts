...
@Component({
  selector: 'app-dialog-vendor-waiter',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-vendor-waiter.html',
})
export class DialogVendorWaiter {
  ...
  constructor(
    ...
    this.waiterForm.controls['restaurant'].setValue(this.util.getItem('_vendorId'));
    this.waiterForm.controls['countryCode'].setValue(this.api.defaultCountryCode);
  ...
  getInfo() {
    ...
    this.api.get_private('v1/vendor_web/waiter/getById/' + this.id).subscribe({
      next: (response: any) => {
        ...
        if (response && response.info && response.info.id) {
          ...
          console.log(info);
          this.userId = info.id;
          (this.waiterForm as any).removeControl('password');
          (this.waiterForm as any).removeControl('countryCode');
          (this.waiterForm as any).removeControl('mobile');
          (this.waiterForm as any).removeControl('email');
          (this.waiterForm as any).removeControl('restaurant');
  ...
  onCountryCodeSelect(event: MatAutocompleteSelectedEvent) {
    ...
    if (event && event.option && event.option.value) {
      ...
      if (splitString && splitString instanceof Array && splitString.length) {
        this.waiterForm.controls['countryCode'].setValue(splitString[1]);
      }
    }
  }
  ...
  onSave() {
    ...
    this.api.post_private('v1/vendor_web/waiter/save', sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
        this.util.onSuccess('ts_waiter_added');
        this.location.back();
      }, error: (error: any) => {
        this.util.stop(spinnerRef);
        this.util.handleError(error, 'vendor');
  ...
  onUpdate() {
    ...
    const sendData = this.waiterForm.getRawValue();
    const spinnerRef = this.util.start('ts_saving');
    this.api.patch_private('v1/vendor_web/waiter/updateWaiterInfo/' + this.userId, sendData).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.stop(spinnerRef);
  ...
  onReset() {
    ...
    this.waiterForm.patchValue({
      ...
      firstName: '',
      lastName: '',
      countryCode: this.api.defaultCountryCode || '',
      mobile: '',
      image: '',
