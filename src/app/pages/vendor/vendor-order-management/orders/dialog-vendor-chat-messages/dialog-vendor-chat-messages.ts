...
import { VendorRegularChatConversionInterface } from 'src/app/interfaces/vendor.regular.chat.conversion.interface';
...
@Component({
  selector: 'app-dialog-vendor-chat-messages',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgScrollbarModule],
  templateUrl: './dialog-vendor-chat-messages.html',
})
export class DialogVendorChatMessages {
  ...
  roomID: string = '';
  chats: VendorRegularChatConversionInterface[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogVendorChatMessages>,
  ...
  getChatMessage() {
    ...
    this.isLoading = true;
    this.api.post_private('v1/vendor_web/chat_room/fetch_messages/', param).subscribe({
      next: (response: any) => {
  ...
  loadMoreMessages() {
    if (this.serverResultChatMessage < this.totalResultChatMessage) {
      ...
      this.api.get_private('v1/vendor_web/regular_chat_messages/' + this.roomID + '?' + httpParams.toString()).subscribe({
        next: (response: any) => {
          console.log(response);
  ...
  sendMessage() {
    ...
    if (this.msg && this.msg != null && this.msg != '') {
      ...
      this.msg = '';
      this.api.post_private('v1/vendor_web/chat_room/send_regular_message/', serverParam).subscribe({
        next: (response: any) => {
          console.log(response);
        }, error: (error: any) => {
          console.log(error);
