...
import { VendorRejectOrderReasonListInterface } from 'src/app/interfaces/vendor.order.reject.order.reason.list.interface';
...
@Component({
  selector: 'app-dialog-reject-order-reason',
  imports: [MaterialModule, CommonModule, FormsModule, ReactiveFormsModule],
  templateUrl: './dialog-reject-order-reason.html',
})
export class DialogRejectOrderReason {

  rejectForm = new FormGroup({
    id: new FormControl('', [Validators.required]),
  });
  reasons: Observable<VendorRejectOrderReasonListInterface[]>;
  listOfReasons: VendorRejectOrderReasonListInterface[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogRejectOrderReason>,
  ) {
    this.getReason();
  }
  ...
  getReason() {
    ...
    this.api.get_private('v1/vendor_web/cancellation/restaurant/').subscribe({
      next: (response: any) => {
  ...
  private _filterReason(value: any): VendorRejectOrderReasonListInterface[] {
  ...
  getTranslatedReasonName(item: VendorRejectOrderReasonListInterface): string {
