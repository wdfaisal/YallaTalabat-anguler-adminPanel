...
@Component({
  selector: 'app-vendor-order-invoice',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './vendor-order-invoice.html',
})
export class VendorOrderInvoice {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/vendor_web/orders/print_invoice/' + this.id + '/' + this.util.getItem('_vendorId')).subscribe({
      next: (response: any) => {
        ...
        if (response && response.success == true) {
          ...
          if (details.scheduleOrder == true) {
            const date = DateTime.fromISO(details.scheduleDate).setLocale(this.util.appLocaleName()).toFormat('yyyy-MM-dd');
            this.orderDateTime = `${date} ${details.scheduleTime}`;
          } else {
            const date = DateTime.fromISO(details.createdAt).setLocale(this.util.appLocaleName()).toFormat('yyyy-MM-dd');
            this.orderDateTime = `${date} ${details.orderAt}`;
          }

          if (details && details.orderTo == 'homedelivery' && details.deliveryAddressRaw && details.deliveryAddressRaw.id) {
            this.deliveryAddress = `${details.deliveryAddressRaw.flatHouse}, ${details.deliveryAddressRaw.locality}, ${details.deliveryAddressRaw.landmark}`;
          } else {
            this.deliveryAddress = this.util.appTranslate('takeaway');
          }

          if (details && details.paymentInfo && details.paymentInfo.id && details.paymentInfo.id != '') {
            this.paymentName = details.paymentInfo.name;
            if (details && details.paymentInfo && details.paymentInfo.translations && Array.isArray(details.paymentInfo.translations)) {
              if (details.paymentInfo.translations) {
                const translation = details.paymentInfo.translations.find((t: any) => t.code == this.util.appLocaleName());
                this.paymentName = translation?.value || details.paymentInfo.name;
              }
            }
          }

          this.receiverName = details.receiverName;
          this.receiverContact = `+${details.countryCode} ${details.receiverContact}`;
          ...
          if (details && details.deliveryCharge && details.deliveryCharge > 0) {
            const obj = {
              "name": this.util.appTranslate('delivery_fee'),
              "option": [''],
              "price": ` + ${this.util.priceFormat(details.deliveryCharge)}`,
            };
            this.priceList.push(obj);
          }
          ...
          if (details && details.couponDiscountCharge && details.couponDiscountCharge > 0) {
            const obj = {
              "name": this.util.appTranslate('th_coupon_discount'),
              "option": [''],
              "price": ` - ${this.util.priceFormat(details.couponDiscountCharge)}`,
            };
            this.priceList.push(obj);
          }
          if (details && details.walletAmount && details.walletAmount > 0) {
            const obj = {
              "name": this.util.appTranslate('th_wallet_amount'),
              "option": [''],
              "price": ` - ${this.util.priceFormat(details.walletAmount)}`,
            };
            this.priceList.push(obj);
          }
          if (details && details.deliveryTip && details.deliveryTip > 0) {
            const obj = {
              "name": this.util.appTranslate('th_delivery_tip'),
              "option": [''],
              "price": ` + ${this.util.priceFormat(details.deliveryTip)}`,
            };
            this.priceList.push(obj);
          }
    ...
    const hardcodedHTML = `
      <!DOCTYPE html>
        ...
        <body>
          ...
          <p class="lbl" style="width:${printPaddingWidth}">${this.util.appTranslate('address')}: ${this.deliveryAddress}</p>
          ...
          <p class="lbl-center" style="width:${printPaddingWidth}"><span>&#169;</span> ${this.currentYear} ${this.appName},
              ${this.util.appTranslate('all_right_reserved')}</p>
