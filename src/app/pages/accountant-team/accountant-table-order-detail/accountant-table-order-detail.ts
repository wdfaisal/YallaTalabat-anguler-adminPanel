...
@Component({
  selector: 'app-accountant-table-order-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './accountant-table-order-detail.html',
})
export class AccountantTableOrderDetail {
  ...
  getOrderDetail() {
    ...
    this.api.get_private('v1/accountant/table_order_detail/' + this.id).subscribe({
      next: (response: any) => {
        ...
        if (response && response.success == true) {
          ...
          if (detail && detail.tableDetail && detail.tableDetail.id && detail.tableDetail.id != '') {
            this.tableNo = detail.tableDetail.tableNumber;
          }
        }
      }, error: (error: any) => {
        console.log(error);
