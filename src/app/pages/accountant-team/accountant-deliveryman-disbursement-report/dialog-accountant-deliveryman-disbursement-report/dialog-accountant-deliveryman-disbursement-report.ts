...
@Component({
  selector: 'app-dialog-accountant-deliveryman-disbursement-report',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './dialog-accountant-deliveryman-disbursement-report.html',
})
export class DialogAccountantDeliverymanDisbursementReport {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogAccountantDeliverymanDisbursementReport>,
  ...
  getDetail() {
    ...
    this.api.get_private('v1/accountant/deliveryman_disbursement_detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onAccept() {
    console.log('Accept');
    this.isSubmit = true;
    this.api.get_private('v1/accountant/accept_deliveryman_disbursement/' + this.id).subscribe({
      next: (response: any) => {
        console.log(response);
