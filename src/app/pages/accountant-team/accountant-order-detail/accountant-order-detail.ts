...
import { DialogAccountantTeamChatMessage } from 'src/app/pages/accountant-team/dialog-accountant-team-chat-message/dialog-accountant-team-chat-message';
...
@Component({
  selector: 'app-accountant-order-detail',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgIcon],
  templateUrl: './accountant-order-detail.html',
})
export class AccountantOrderDetail {
  ...
  getInfo() {
    ...
    this.api.get_private('v1/accountant/regular_order_detail/' + this.id).subscribe({
      next: (response: any) => {
  ...
  onUserInfo() {
    if (this.userId && this.userId != '') {
      this.router.navigate(['accountant-team/u/customer-detail', this.userId]);
    }
  }

  onRestaurantDetail() {
    if (this.restaurantId && this.restaurantId != '') {
      this.router.navigate(['/accountant-team/u/restaurant-detail', this.restaurantId]);
    }
  }

  onDeliverymanDetail() {
    if (this.driverId && this.driverId != '') {
      this.router.navigate(['accountant-team/u/deliveryman-detail', this.driverId]);
    }
  }
        ...
        next: (response: any) => {
          ...
          if (response && response.success) {
            ...
            if (kind == 'call') {
              ...
              this.dialog.open(DialogAccountantTeamChatMessage, {
                disableClose: true,
                data: { id: `${id}`, name: `${name}`, email: `${email}`, countryCode: `${countryCode}`, mobile: `${mobile}`, role: `${role}`, image: `${image}` },
