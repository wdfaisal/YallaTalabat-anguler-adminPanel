...
import { AccountantRegularOrderReportInterface } from 'src/app/interfaces/accountant.regular.order.interface';
...
@Component({
  selector: 'app-accountant-regular-order-report',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, MatNativeDateModule],
  templateUrl: './accountant-regular-order-report.html',
})
export class AccountantRegularOrderReport {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  reports = new MatTableDataSource<AccountantRegularOrderReportInterface>([]);
  displayedColumn = ['id', 'customer', 'restaurant', 'payment', 'real_total', 'item_discount', 'item_total', 'coupon_discount', 'delivery_charge', 'food_tax', 'service_tax', 'package_charge', 'package_charge_tax', 'delivery_tip', 'extra_charge', 'wallet_amount', 'grand_total', 'refunded_amount', 'deliveryman_earning', 'deliveryman_commission', 'restaurant_commission', 'status', 'date', 'action'];
  pageSize: number = 5;
  ...
  getList() {
    ...
    this.api.get_private('v1/accountant/regular_order_report?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AccountantRegularOrderReportInterface) => {
              ...
              if (item && item.paymentInfo && item.paymentInfo?.id) {
                if (item.paymentInfo?.translations) {
                  const translation2 = item.paymentInfo.translations.find((t) => t.code == this.util.appLocaleName());
                  item.paymentInfo.displayName = translation2?.value || item.paymentInfo.name;
                } else {
          ...
          this.reports = new MatTableDataSource<AccountantRegularOrderReportInterface>(mappedList);
  ...
  receivedBy(order: AccountantRegularOrderReportInterface) {
    if (order.paymentMode == 'online' && order.status == 'delivered') {
      return this.util.appTranslate('received_by_admin');
    } else if (order.paymentMode == 'offline' && order.orderTo == 'homedelivery' && order.status == 'delivered') {
      return this.util.appTranslate('received_by_deliveryman');
    } else if (order.paymentMode == 'offline' && order.orderTo != 'homedelivery' && order.status == 'delivered') {
      return this.util.appTranslate('received_by_restaurant');
    } else if ((order.paymentMode == 'offline' || order.paymentMode == 'online') && order.status == 'refunded') {
      return this.util.appTranslate('refunded');
    } else if (order.paymentMode == 'offline' && order.status != 'delivered') {
      return this.util.appTranslate('not_received');
    }
    return this.util.appTranslate('received_by_admin');
  }
  ...
  onOrderDetailPage(orderId: string) {
    ...
    this.router.navigate(['/accountant-team/u/order-detail', orderId]);
  }
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/accountant/reports/orders/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
  ...
  onUserInfo(item: AccountantRegularOrderReportInterface) {
    console.log(item);
  ...
  onRestaurantDetail(item: AccountantRegularOrderReportInterface) {
    console.log(item);
