...
import { AccountantPosOrderReportInterface } from 'src/app/interfaces/accountant.pos.order.report.interface';
...
@Component({
  selector: 'app-accountant-pos-order-report',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, MatNativeDateModule],
  templateUrl: './accountant-pos-order-report.html',
})
export class AccountantPosOrderReport {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  reports = new MatTableDataSource<AccountantPosOrderReportInterface>([]);
  displayedColumn = ['id', 'customer', 'restaurant', 'payment', 'real_total', 'item_discount', 'item_total', 'discount', 'food_tax', 'service_tax', 'package_charge', 'package_charge_tax', 'waiter_tip', 'extra_charge', 'grand_total', 'commission', 'status', 'date', 'action'];
  pageSize: number = 5;
  ...
  getList() {
    ...
    this.api.get_private('v1/accountant/pos_order_report?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AccountantPosOrderReportInterface) => {
          ...
          this.reports = new MatTableDataSource<AccountantPosOrderReportInterface>(mappedList);
  ...
  getStatusColor(orderStatus: string) {
    ...
    const successOrderStatus = ['ready', 'completed'];
  ...
  onOrderDetailPage(orderId: string) {
    console.log(orderId);
    this.router.navigate(['/accountant-team/u/pos-order-detail', orderId]);
  }
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/accountant/reports/pos_orders/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          const fileName = exportOption == 'excel' ? 'POSOrders.xlsx' : 'POSOrders.csv';
  ...
  onRestaurantDetail(item: AccountantPosOrderReportInterface) {
    console.log(item);
