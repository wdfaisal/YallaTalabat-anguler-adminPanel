...
@Component({
  selector: 'app-accountant-restaurant-disbursement-report',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, MatNativeDateModule, NgIcon],
  templateUrl: './accountant-restaurant-disbursement-report.html',
})
export class AccountantRestaurantDisbursementReport {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  reports = new MatTableDataSource<AccountantRestaurantDisbursementReportListInterface>([]);
  ...
  getInitial() {
    ...
    this.api.get_private('v1/accountant/restaurant_initial_disbursement').subscribe({
      next: (response: any) => {
        this.util.stop(spinnerRef);
        console.log(response);
        if (response && response.cities) {
          this.listOfCities = response.cities.map((city: AccountantCityListInterface) => {
  ...
  getList() {
    ...
    this.api.get_private('v1/accountant/restaurant_disbursement?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          ...
          this.reports = new MatTableDataSource<AccountantRestaurantDisbursementReportListInterface>(mappedList);
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/accountant/reports/restaurant_disbursement/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
