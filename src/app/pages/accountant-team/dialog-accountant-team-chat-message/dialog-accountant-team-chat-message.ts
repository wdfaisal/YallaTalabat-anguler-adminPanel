...
import { AccountantRegularChatConversionInterface } from 'src/app/interfaces/accountant.regular.chat.conversion.interface';
...
@Component({
  selector: 'app-dialog-accountant-team-chat-message',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgScrollbarModule],
  templateUrl: './dialog-accountant-team-chat-message.html',
})
export class DialogAccountantTeamChatMessage {
  ...
  roomID: string = '';
  chats: AccountantRegularChatConversionInterface[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogAccountantTeamChatMessage>,
  ...
  getChatMessage() {
    ...
    this.isLoading = true;
    this.api.post_private('v1/accountant/chat_room/fetch_messages/', param).subscribe({
      next: (response: any) => {
  ...
  loadMoreMessages() {
    if (this.serverResultChatMessage < this.totalResultChatMessage) {
      ...
      this.api.get_private('v1/accountant/regular_chat_messages/' + this.roomID + '?' + httpParams.toString()).subscribe({
        next: (response: any) => {
          console.log(response);
  ...
  sendMessage() {
    ...
    if (this.msg && this.msg != null && this.msg != '') {
      ...
      this.msg = '';
      this.api.post_private('v1/accountant/chat_room/send_regular_message/', serverParam).subscribe({
        next: (response: any) => {
          console.log(response);
        }, error: (error: any) => {
          console.log(error);
