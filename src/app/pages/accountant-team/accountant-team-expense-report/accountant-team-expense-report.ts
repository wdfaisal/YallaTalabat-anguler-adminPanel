...
import { AccountantExpenseReportInterface } from 'src/app/interfaces/accountant.expense.report.interface';
import { DialogAddAccountantExpense } from './dialog-add-accountant-expense/dialog-add-accountant-expense';
...
@Component({
  selector: 'app-accountant-team-expense-report',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, NgApexchartsModule, NgIcon],
  templateUrl: './accountant-team-expense-report.html',
})
export class AccountantTeamExpenseReport {

  @ViewChild('expenseChart') expenseChart: ChartComponent = Object.create(null);
  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  reports = new MatTableDataSource<AccountantExpenseReportInterface>([]);
  ...
  getInitial() {
    ...
    this.api.get_private('v1/accountant/expense_initial?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        ...
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AccountantExpenseReportInterface) => {
              if (item && item.couponInfo && item.couponInfo?.id) {
                if (item.couponInfo?.translations) {
                  ...
                  item.couponInfo.displayName = translation?.name || item.couponInfo.name;
                } else {
              ...
              if (item && item.diningCouponInfo && item.diningCouponInfo?.id) {
                if (item.diningCouponInfo?.translations) {
                  const translation2 = item.diningCouponInfo.translations.find((t) => t.code == this.util.appLocaleName());
                  item.diningCouponInfo.displayName = translation2?.name || item.diningCouponInfo.name;
                } else {
          ...
          this.reports = new MatTableDataSource<AccountantExpenseReportInterface>(mappedList);
  ...
  getList() {
    ...
    this.api.get_private('v1/accountant/expense?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
  ...
  addExpsense() {
    const dialogRef = this.dialog.open(DialogAddAccountantExpense, {
      disableClose: true
    });
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/accountant/expense/export/' + exportOption + '/' + this.expenseType).subscribe({
        next: (blob: Blob) => {
  ...
  onOrderInfo(order: AccountantExpenseReportInterface) {
    console.log(order);
    this.router.navigate(['accountant-team/u/order-detail', order.orderInfo.id]);
  }

  onBookingInfo(booking: AccountantExpenseReportInterface) {
    console.log(booking);
    this.router.navigate(['accountant-team/u/dining-booking-detail', booking.bookingInfo.id]);
  }

  onCouponInfo(coupon: AccountantExpenseReportInterface) {
    console.log(coupon);
  }

  onDiningCouponInfo(coupon: AccountantExpenseReportInterface) {
    console.log(coupon);
  }

  onUserInfo(item: AccountantExpenseReportInterface) {
    console.log(item);
