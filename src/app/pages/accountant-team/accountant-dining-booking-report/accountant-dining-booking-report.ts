...
import { AccountantDiningBookingReportInterface } from 'src/app/interfaces/accountant.dining.booking.report.interface';
...
@Component({
  selector: 'app-accountant-dining-booking-report',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule, MatNativeDateModule],
  templateUrl: './accountant-dining-booking-report.html',
})
export class AccountantDiningBookingReport {

  @ViewChild('paginator', { read: MatPaginator, static: false }) paginator: MatPaginator;
  reports = new MatTableDataSource<AccountantDiningBookingReportInterface>([]);
  displayedColumn = ['id', 'customer', 'restaurant', 'payment', 'pre_booking_charge', 'coupon_cover_charge', 'grand_total', 'restaurant_commission', 'date', 'status', 'action'];
  pageSize: number = 5;
  ...
  getList() {
    ...
    this.api.get_private('v1/accountant/dining_booking_report?' + httpParams.toString()).subscribe({
      next: (response: any) => {
        console.log(response);
        this.isLoaded = true;
        if (response && response.results) {
          const mappedList = response.results.map(
            (item: AccountantDiningBookingReportInterface) => {
          ...
          this.reports = new MatTableDataSource<AccountantDiningBookingReportInterface>(mappedList);
  ...
  onBookingDetailPage(bookingId: string) {
    ...
    this.router.navigate(['/accountant-team/u/dining-booking-detail', bookingId]);
  }
  ...
  onExportCollection(exportOption: string) {
    ...
    if (exportOption != 'export') {
      ...
      this.api.export_collection('v1/accountant/reports/dining_booking/export?' + httpParams.toString()).subscribe({
        next: (blob: Blob) => {
          this.util.stop(spinnerRef);
          const fileName = exportOption == 'excel' ? 'DiningBookingReport.xlsx' : 'DiningBookingReport.csv';
  ...
  onUserInfo(item: AccountantDiningBookingReportInterface) {
  ...
  onRestaurantDetail(item: AccountantDiningBookingReportInterface) {
    console.log(item);
