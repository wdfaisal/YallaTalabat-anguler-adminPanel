...
import { VendorHeaderChatListInterface } from 'src/app/interfaces/vendor.header.chat.list.interface';
import { VendorNotificationListInterface } from 'src/app/interfaces/vendor.notification.list.interface';
...
export class VendorHeaderComponent {
  ...
  chatList: VendorHeaderChatListInterface[] = [];
  unreadMessages: number = 0;
  notifications: VendorNotificationListInterface[] = [];
  ...
  profiledd: profiledd[] = [
    {
      ...
      link: '/vendor/profile-management/profile-setting',
    },
    {
      id: 2,
      ...
      link: '/vendor/profile-management/vendor-chat-list',
    },
    {
      id: 3,
      ...
      link: '/vendor/profile-management/notification-list',
    },
  ];
  ...
  constructor(
    ...
    if (this.util.getItem('_authRole') == 'vendor') {
      this.role = this.util.appTranslate('restaurant');
    } else {
      this.role = this.util.appTranslate('outlet');
    }
  ...
  headerContent() {
    this.api.get_private('v1/vendor_web/vendor_header_content/' + this.util.getItem('_uid')).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response && response.success) {
          const mappedList = response.notifications.map(
            (item: VendorNotificationListInterface) => {
              let username: string = '';
  ...
  onNotification() {
    ...
    this.router.navigate(['/vendor/profile-management/notification-list']);
  }

  onChat() {
    console.log('Navigate');
    this.router.navigate(['/vendor/profile-management/vendor-chat-list']);
  }
