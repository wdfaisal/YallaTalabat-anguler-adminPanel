...
import { SupportTeamHeaderChatListInterface } from 'src/app/interfaces/support.team.header.chat.list.interface';
...
export class SupportTeamHeaderComponent {
  ...
  profiledd: profiledd[] = [
    {
      ...
      link: '/support-team/u/profile-setting',
    },
  ];
  chatList: SupportTeamHeaderChatListInterface[] = [];
  ...
  headerContent() {
    this.api.get_private('v1/support_team/support_team_header_content/' + this.util.getItem('_uid')).subscribe({
      next: (response: any) => {
  ...
  onChat(item: SupportTeamHeaderChatListInterface) {
    console.log(item);
...
@Component({
  selector: 'search-support-team-dialog',
  imports: [
    RouterModule,
    MaterialModule,
    NgIcon,
