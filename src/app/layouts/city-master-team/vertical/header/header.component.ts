...
import { CityzenNotificationListInterface } from 'src/app/interfaces/cityzen.notification.list.interface';
import { CityzenHeaderSupportChatListInterface } from 'src/app/interfaces/cityzen.header.support.chat.list.interface';
...
@Component({
  selector: 'app-city-master-team-header',
  imports: [
    RouterModule,
    CommonModule,
    NgScrollbarModule,
...
export class CityMasterTeamHeaderComponent {
  ...
  notifications: CityzenNotificationListInterface[] = [];
  ...
  profiledd: profiledd[] = [
    {
      ...
      link: '/cityzen-team/profile-setting',
    },
    {
      id: 2,
    ...
    },
    {
      ...
      link: '/cityzen-team/notification-list',
    },
  ];
  chatList: CityzenHeaderSupportChatListInterface[] = [];
  ...
  headerContent() {
    this.api.get_private('v1/cityzen/cityzen_team_header_content/' + this.util.getItem('_uid')).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response && response.success) {
          this.unreadMessages = response.unread;
          const mappedList = response.notifications.map(
            (item: CityzenNotificationListInterface) => {
              let username: string = '';
          ...
          this.chatList = response.supportChat;
  ...
  onLogout() {
    ...
    this.api.post_public('v1/auth/logout_web', {}).subscribe({
      next: (response: any) => {
        console.log(response);
        this.util.clearItem();
        this.router.navigate(['/authentication/cityzen']);
      }, error: (error: any) => {
        console.log(error);
        this.util.clearItem();
        this.router.navigate(['/authentication/cityzen']);
  ...
  onChat(item: CityzenHeaderSupportChatListInterface) {
    console.log(item);
    this.router.navigate(['/cityzen-team/chat-list', item.id]);
  }

  onNotification() {
    this.router.navigate(['/cityzen-team/notification-list']);
  }
...
@Component({
  selector: 'search-city-master-dialog',
  imports: [
    RouterModule,
    MaterialModule,
    NgIcon,
